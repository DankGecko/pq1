...
/// body. Returns `(elems_start, count)` — the byte offset of the first
/// element word and the element count — or `Err` (decline-to-blind) on ANY
/// malformed / hostile body. Pure: no rendering, so it is independently
/// testable + Kani-verifiable.
pub fn resolve_array(
    ...
    // 4. EXACT placement: the array tail starts exactly at head-end.
    if offset != head_end {
    ...
    // 6. EXACT placement: the array is the ENTIRE dynamic tail (no slack).
