...
This package binds the six PQ1-admitted Morpho Blue `borrow`, `withdraw`,
`withdrawCollateral`, `supply`, `repay`, and `supplyCollateral` displays to
the canonical same-address deployments on Ethereum and Base.
...
The six admitted routes show all five words that define the market, both
asset/share input words where present, `onBehalf`, and either `receiver` or an
exact-empty callback witness. The curation formats `assets` as a token amount
using the exact signed market token:

- `borrow`, `withdraw`, `supply`, and `repay`: `marketParams.loanToken`;
- `withdrawCollateral` and `supplyCollateral`:
...
`supply`, `repay`, and `supplyCollateral` are admitted only when `data` is the
sole canonical dynamic `bytes` tail, its length word is exactly zero, and its
padded end is exactly calldata EOF. In that case PQ1 displays `Callback: none`.
Any non-empty callback payload, aliased or misaligned offset, trailing bytes,
or otherwise malformed ABI encoding remains refused before any signing pages.
This does not add a renderer or signing authority for arbitrary callback data.
