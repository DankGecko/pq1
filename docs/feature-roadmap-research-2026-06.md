...
| **DeFi lending (Aave v3 / Spark / Morpho Blue)** | Clear-sign `Pool.supply/withdraw/borrow/repay` + Morpho direct `setAuthorization`; assert `onBehalfOf==this` | Both | M | High | ERC-7730 pack | Use Morpho's **direct** `setAuthorization`, never `setAuthorizationWithSig` (ECDSA). Grounds the backlogged ZK-Aave item; Groth16 binding is optional XL hardening, not required |
...
| **In-world Groth16/BLS12-381 intent-binding for privacy UserOps** | Reuse the CoW-pattern verifier to bind a human-readable privacy intent to an opaque UserOp before signing | Both | L | Med | new attestation circuit | The only HW wallet that could cryptographically verify on-device what a privacy tx does. Needs a new public-input schema + companion-side attestation circuit (the heavy part) |
