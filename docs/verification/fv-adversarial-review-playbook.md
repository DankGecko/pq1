...
| V9 | **Model ≠ deployed artifact** | Theorem about a Lean model that doesn't faithfully mirror the bytecode/firmware/transcription | the LeanModel.sol ↔ Lean TCB; Charon-LLBC | **Differential KAT** (same vectors through model + artifact) + codegen the bridge + an **independent engine** (Kontrol vs Halmos). Not a pure-Lean check. **For wire-protocol models (added 2026-07-17, M3 lesson): require a model re-derived from an external oracle (vendor spec/reference implementation), not only from the driver it models** — a driver-derived model can at best prove self-consistency. M3: the driver-derived `optiga_shield_handshake.pv` invented a host nonce the PRL silicon doesn't have and answered injective agreement TRUE; the vendor re-derivation (`optiga_shield_handshake_vendor.pv`, pinned FALSE by `check_protocol_models.py`) caught it. Companion durability gap (no mechanical home yet): when a model gains a KNOWN-INFIDELITY banner, nothing forces its citers (README, THREAT_CLAIM_MAP rows) to update — audit the citer set when a model's honesty status changes | ⚠ partial (Aeneas diff-gate, Kontrol) |
...
1. **Proof-mutation gate — ✅ BUILT (`make verify-proof-mutation`).** The proof-side analogue of `cargo-mutants`: `contracts/verification/scripts/check_proof_mutations.py` drives the manifest `lean/scripts/proof_mutations.json`, deleting (by rename) each load-bearing axiom and weakening key lemmas, then asserting the rebuild **reacts as the ledger claims** — a load-bearing axiom's removal must break the build; a non-consumed A1/A4 marker's removal must stay green AND drop *exactly* the advertised axiom from `theft_free`'s closure; the zero-consumer A3.4 must stay green; the P1 `reachable_implies_combinedCap` must be load-bearing for the reachable headline. Each mutation asserts it **materially changed the file** (a no-op is a HARD FAIL, never a skip), does a full transitive rebuild (no stale-cache false pass), and **always reverts**; a permanent **canary** must trip or the harness is declared void. It *executes the ledger's own falsifiability prose* — the prose claims become mechanical tests. The authoritative `make verify-proof-mutation` target is pinned to the curated default tier; use the direct checker with explicit `--tier quick|default|full` only for clearly labelled exploratory/full local runs.
2. **Ledger-consistency gate — ✅ BUILT (`verify-ledger-consistency`; direct Make target is diagnostic).** `contracts/verification/scripts/check_ledger_consistency.py` makes `AXIOM_STATUS.json` falsifiable against the live Lean truth: a machine-readable `closures` block (exact-set per theorem) + `signature_pins` (headline statement-shape — catches a re-introduced raw `hInv` that closure checks can't see) are diffed against `dump_axioms.lean`, the Lean source, the per-status summary counts, and the `lint_fv` THEFT_EXPECTED pin (single authoritative source — no 5th drift surface). C16 pins the exact top-level schema and full canonical ledger value, plus the granular axiom-row and discharge-artifact schemas/values; this binds corollaries, witness descriptions, headlines, and other assurance prose as well as IDs/artifacts, and duplicate artifacts fail. Plus no-undocumented / no-phantom axiom + status hygiene. Ships a wired-in `--self-test` negative control, including identity-swap, artifact duplication, appended-corollary, witness-substitution, and headline-substitution attacks. Authoritative local/CI use goes through `contracts/verification/scripts/run_authoritative_make.sh` under the non-root, same-UID setuid-sudo → `/usr/bin/env -i` boundary documented in `THREE_CLAIMS_PROOF.md`; CI executes setuid sudo directly as an exact-pinned custom shell before Bash and `exec`s pinned Bash on the launcher. The G1 contract pins the GitHub-hosted runner, forbids containers and job startup/skip controls, rejects duplicate YAML/JSON keys, and self-tests runner/container/conditional/no-op/startup-state suppression. Secure-execution startup crosses the loader boundary before dropping back to the caller, and the launcher then rejects leaked environment, resolves its physical single-link file identity before deriving the root, accepts only this target, and requires its exact completion marker. Loader suppression, exported-function interception, and symlink/hard-link lookalikes therefore cannot manufacture success before the real gate runs. *Caveat in the header:* `#print axioms` under-reports in lean v4.22.0; `make verify-lean4checker` is the completeness backstop — this gate guards advertised-vs-`#print-axioms` consistency, not the under-report gap. (Already surfaced + fixed one real gap on first run: an undocumented `solidityWalletExecuteBatch` axiom.)
3. **Enforced non-vacuity-witness coverage (closes V1/V6 systematically) — ✅ BUILT (`verify-ledger-consistency` C9, 2026-06-29).** The `*_nonvacuous` convention is now a *requirement*: the ledger's `witness_coverage` block pins each headline hypothesis to a hand-written witness lemma (e.g. `combinedCapInvariant_empty` witnesses `theft_free_bytecode`'s `hInv`; `H_adrs/H_sib_dischargeable` the FORS-climb bounds; `execute_step_satisfiable` the Claim-4 credit), and C9 fails CI if a witness drops from `dump_axioms.lean` OR its `#print axioms` closure leaves **kernel-only** (a witness resting on a project axiom could be circular — witnessing the very assumption). **Adapted, not literally ported,** from LeanLoop's `vet` HYP probe: LeanLoop finds witnesses mechanically with `plausible`, but PQSigner's `lean/` is deliberately **mathlib-free** (the load-bearing invariant that makes a false axiom non-detonatable — pulling in Plausible would break it), so we enforce the *existing hand-witnesses* instead. (This C9 gate is kernel-checked and does **not** rest on any LeanLoop `vet` verdict — relevant because `vet`'s RED/`NEGATION PROVED` classification is itself unreliable until fixed; see F10 / `spec-assurance-research-2026-06.md`.) Residual: the witness set is the headline-hypothesis scope, not yet *every* predicate (the two `private` C10 interpreter witnesses aren't externally pinnable and stay covered by the proof-mutation full rebuild).

4. **Gate-enforcement lint — ✅ BUILT 2026-07-01 (`make verify-gate-enforcement`; closes G1).** The 2026-07-01 round's HIGH finding (F1) is that a gate can be green-when-run yet never RUN. That is a *mechanical* class — exactly what Layer 1 automates — so leaving it to the adversary would itself be a G1 (a prose gate that never executes). `scripts/check_gate_enforcement.py` + manifest `scripts/gate_enforcement.json`: each soundness gate declares what surface it polices + where it must run; the checker parses duplicate-key-free workflow YAML + manifest JSON and asserts (a) a job invokes it, (b) its trigger `paths:` cover the policed surface (the F1 check), (c) its push/PR branch filters cover protected `master`, and (d) it is blocking (no `continue-on-error` / not `workflow_dispatch`-only), else `enforcement=local_documented` with a `why` (a NOTE, never silent). Ordered negative path/branch patterns are rejected fail-closed instead of approximated. Load-bearing workflow steps can additionally pin their complete parsed step, trusted runner, forbidden job context, and their workflow/launcher/checker/Makefile trigger surface, so target text moved to self-hosted/container execution, hidden behind `if: false`, a no-op custom shell, hostile startup environment, or a self-suppressing trigger does not count as execution. Fail-closed; ships `--self-test` negative controls for those cases. **Validated:** passes the current tree; the self-test catches a broken gate, nonmatching/negative branch filters, ordered path exclusion, missing workflow self-path, and the exact F1 when `docs/**` is stripped from `lean-fv.yml`. Wired per-PR in `ci.yml` (a G1 lint that is not itself CI-enforced would be its own G1).
...
- **Per-commit:** the Layer-1 gates — now including `make verify-proof-mutation` + `make verify-ledger-consistency` (BUILT, incl. the C9 enforced-witness coverage). (proof-mutation is slow → run it on FV-touching PRs / nightly, not every commit; ledger-consistency is fast.)
