...
is ~2 of the 8 FV surfaces (see contracts/verification/docs/FV_SURFACE_MAP.md). The
...
1. **Surface scope — the review reaches ~2 of 8 FV surfaces.** The master-prompt TARGET is the Lean on-chain tree; [`FV_SURFACE_MAP.md`](../../contracts/verification/docs/FV_SURFACE_MAP.md) enumerates the whole surface, and firmware Kani / Miri / protocol models / CT-SCA / Aeneas §33 / differential-fuzz have **never had a first adversarial pass**. Every "no hole found" is scoped to what was targeted — never the whole stack. Extending the TARGET (the surface note in Part C) is the standing next-round work.
