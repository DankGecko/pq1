...
3. **Enforced non-vacuity-witness coverage (closes V1/V6 systematically) — ⚠ STILL OPEN.** Make the `*_nonvacuous` convention a *requirement*: a lint that every cited `axiom` and every security predicate (`isForgery`, `consistent`, each `combinedCapInvariant`-style hypothesis) has a companion `∃`-witness lemma proving its hypothesis is satisfiable on the *operational* input space — and that the witness is itself tracked in `dump_axioms`. The last automatable gap.
