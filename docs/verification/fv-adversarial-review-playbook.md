...
|---|---|---|---|---|---|
| G1 | **Gate-enforcement vacuity** | the anti-vacuity gate exists + is green-when-run, but never RUNS on the diff it polices — path-filtered out, `continue-on-error`, `workflow_dispatch`-only, unwired, or not a required check on the protected branch | **FOUND 2026-07-01 (HIGH, F1):** `verify-ledger-consistency` matched no positive `paths:` on ledger-only edits + `ci.yml` path-ignores `contracts/verification/**` + nightly never called it (PoC: 3 merged ledger-only commits); `verify-storage-mutators` unwired; `checkct` is `workflow_dispatch`+`continue-on-error` | **CI-wiring audit** per `verify-*` gate: (a) a job invokes it, (b) its trigger `paths:` cover the surface it polices, (c) it is blocking (no `continue-on-error` / not manual-only), (d) it is a required status check on `master`. PoC: a merged diff that touched the policed surface without triggering the gate. | ✅ a gate-enforcement lint (**proposed, Layer 1 — NOT YET BUILT**) |
...
4. **Gate-enforcement lint — ⛔ NOT YET BUILT (proposed 2026-07-01; closes G1).** The 2026-07-01 round's HIGH finding (F1) is that a gate can be green-when-run yet never RUN. That is a *mechanical* class — exactly what Layer 1 automates — so leaving it to the adversary would itself be a G1 (a prose gate that never executes). Proposed `check_gate_enforcement.py`: parse the `verify-*` targets in `contracts/verification/Makefile` + the root `Makefile` and the workflow files, and assert for each gate (a) a job invokes it, (b) its trigger `paths:` cover the surface it polices, (c) it is blocking (no `continue-on-error` / not `workflow_dispatch`-only), (d) it is a required status check on `master` (via `gh api …/branches/master/protection` where accessible). Fail-closed; ships a negative control (rename a gate's path-trigger → expect RED). **Until built, G1 is caught only by the periodic swarm's CI-wiring audit — which is exactly how F1 surfaced.**
