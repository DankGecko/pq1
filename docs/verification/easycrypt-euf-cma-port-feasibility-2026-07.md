...
**Remaining to `EUFCMA_SPHINCS_PLUS`:** the WOTS+C FC↔STCRC/MM45 bridge (in progress); FORS+C tree-term instantiation against concrete FORS_ES games + FORS+C multi-instance; then compose. These are the "MM45 unification"-flavoured legs — the ones that have proven bug-prone, so they warrant design care, not speed.
