...
**⚠️ PROVISIONAL — currently violated by ship blockers S-1, S-5, S-6 (`docs/work-todo.md`).** Against a *desoldered-OPTIGA bench-rig* attacker (i.e. the threat that motivates the LUC at all), the OPTIGA leg of this claim does not hold in the current build: `F1D0.Change = ALW` (`secure/src/optiga/apdu.rs:930`, `:1059`) lets an attacker overwrite F1D0 with a chosen key, self-auth, and reset E120 indefinitely. The on-device three-way lockstep still bounds an *attacker on the wire* to 10 attempts (firmware-side `gated_unlock` decrements MCU page 124 atomically pre-SE-call, so even with the OPTIGA leg defeated the MCU + SE050 counters still trip), but the LUC layer that this claim attributes to OPTIGA is structurally absent on a chip removed from the board. SE050 UserID's failure-decrement is documented by NXP and survives desoldering, so the dual-SE composition still bounds the attacker — but until S-1 is closed (F1D0 metadata tightened to `Auto(F1D0)` + LcsO ratchet), this claim should be read as "10 wrong PINs against MCU + SE050; OPTIGA contributes belt-and-braces *on-board* only." Re-establishing the original claim requires closing S-1 and re-running `pin-gate-hw-counter-e2e` against a ratcheted sacrificial part with the tightened metadata. **S-5 + S-6 do not affect Claim 3 directly** (the brute-force bound), but they break the *secrecy-of-seed-given-bound-attempts* invariant the threat model relies on for Claim 3 to be load-bearing: S-5 puts `half_E` on the I²C bus in cleartext on every successful unlock; S-6 lets admin compromise impersonate the user and read `half_E` without the original PIN. Both must close before Claim 3 is meaningful.

**Claim 5 (new) — Post-lockout secret state on SE050 is *orphaned*, not erased.** When the SE050 UserID hits `auth_attempts == max_attempts`, the UserID locks (policy denies further VERIFY). The associated ENTROPY_OBJ / MASTER_OBJ / VK_OBJ data remain on the die — they become unreachable because nothing can authenticate against the locked UserID, but the bytes physically persist until a successful `factory_reset_admin` cycle overwrites the slots. `iterative_delete_all` (`apdu.rs:705-764`) is NOT the destroy-on-lockout mechanism — it requires the PIN to fire pass 2 (`:739`); on the actual 10-wrong-PIN scenario, pass 2 bails and the secret survives orphaned. If true erasure on lockout is required (not just orphan), the firmware lockout-detection path must call full-chip `factory_reset` directly, gated on `AuthMethodBlocked`. Tracked under S-6 / S-7 §5 in `docs/work-todo.md`.
