...
**⚠️ PROVISIONAL — currently violated by ship blocker S-1 (`docs/work-todo.md`).** Against a *desoldered-OPTIGA bench-rig* attacker (i.e. the threat that motivates the LUC at all), the OPTIGA leg of this claim does not hold in the current build: `F1D0.Change = ALW` (`secure/src/optiga/apdu.rs:930`, `:1059`) lets an attacker overwrite F1D0 with a chosen key, self-auth, and reset E120 indefinitely. The on-device three-way lockstep still bounds an *attacker on the wire* to 10 attempts (firmware-side `gated_unlock` decrements MCU page 124 atomically pre-SE-call, so even with the OPTIGA leg defeated the MCU + SE050 counters still trip), but the LUC layer that this claim attributes to OPTIGA is structurally absent on a chip removed from the board. SE050 UserID's failure-decrement is documented by NXP and survives desoldering, so the dual-SE composition still bounds the attacker — but until S-1 is closed (F1D0 metadata tightened to `LcsO<Op` + LcsO ratchet), this claim should be read as "10 wrong PINs against MCU + SE050; OPTIGA contributes belt-and-braces *on-board* only." Re-establishing the original claim requires closing S-1 and re-running `pin-gate-hw-counter-e2e` against a ratcheted sacrificial part with the tightened metadata.
