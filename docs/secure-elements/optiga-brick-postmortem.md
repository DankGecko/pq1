...
| ECDSA signing-key masking with OTP-derived mask | We don't sign with OPTIGA's ECC keys; our SPHINCS+C10 signatures are on the STM32. The pattern's worth remembering for work-todo #18 (SLH-DSA SCA/FI hardening), but not directly applicable to the OPTIGA layer. |
...
- Our own prior notes: `docs/secure-elements/optiga-bringup-status.md`, `project_optiga_bringup.md` (memory).
