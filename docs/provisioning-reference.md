...
> `docs/provisioning-research-brief.md`. It is a research *synthesis*, not
...
**Variant: EdgeLock SE050E2, OEF ID `0xA921`** (confirmed — the OM-SE050ARD-E dev board carries SE050E; NXP docs map SE050E2 → OEF 0xA921). Implications baked into the rows below: **CC EAL6+ to OS level (yes), FIPS 140-2 (NO — that's SE050F only), platform SCP03 forced by default (NO — SE050E defaults `SCP_NOT_REQUIRED`; we rely on per-object `REQUIRE_SM`).** ⚠️ Confirm the **production BOM** uses the same SE050E2/0xA921 (or re-evaluate if it ever moves to SE050F for FIPS) — the `GetVersion`/OEF boot assertion (work-todo) should expect `0xA921`.
