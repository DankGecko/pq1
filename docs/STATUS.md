...
- **The real ship gate is NOT a tooling track** — it includes **firmware
  ...
  journal/ECC/OTP receipts remain open), **OPTIGA silicon (S-1/S-3 LcsO ratchet) + S-2 closure of the real type-`0x11` pool and device-certificate retype boundary under a selected factory policy**, the **fresh-TRNG final SE pairing/rotation protocol
  (HIGH-1; current deterministic transport credentials are fenced but are not
  production-final)**, and the **S-5 bus capture**.
- **Keyboard-doable security work right now (`blocked-on: code`):** HIGH-1's
  final pairing protocol and crash-safe durable public state, `make checkct` CI
  enrollment, S-4's remaining
...
| **HIGH-1** | SCP03 default-keyed with PUBLISHED AN12436 factory keys (dev) → bus attacker extracts `half_E` | partial; deterministic transport path fenced, production-final rotation OPEN | **code + factory + bench** | `secure/src/nsc/mod.rs` fence string ``Candidate SE050 profile is incomplete without non-public SCP03 transport keys``; `secure/src/se050/mod.rs::Se050::rotate_scp03_keys`; `secure/src/scp03_logic.rs::keys_are_factory_default` | The published-key fallback is compile-fenced, but that closes only the as-found downgrade. Current code substitutes deterministic BHK-derived credentials; it does **not** implement the required fresh-TRNG final per-device rotation. Freeze and implement the durable public salt/state owner, cut recovery, and coordinated OPTIGA/SE050 rotation contract; then validate PUT KEY, recovery, and E140 ordering on sacrificial silicon. No production authority until all three legs close. |
...
| `docs/security/audits/*-20260611-*.md` | 4 parallel adversarial paper-audits | se-tunnels HIGH-1 (SCP03 factory keys) — published-key fallback is **code-fenced since the audit** (`secure/src/nsc/mod.rs`, fence string ``Candidate SE050 profile is incomplete without non-public SCP03 transport keys``), but the fresh-TRNG final pairing protocol remains code + factory + silicon work; tz-tamper MED-2 (prod-config gate, closed by `f8effd45`); rest resolved-in-commit | The 2026-06-11 four-domain audit record (as-found — verify against current code) |
