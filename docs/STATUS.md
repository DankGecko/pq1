...
| EasyCrypt (A5, WOTS+C leg) | **WOTS+C multi-instance EU-naCMA MACHINE-CHECKED** against MM45's *real* WOTS-TW theorem: `Pr[M_EUF_NACMA_WOTSC_L] ≤ Pr[S_TCR_C] + Pr[M_EUF_GCMA_WOTSTWESNPRF]` — real games, **no free reals, 0 admit**. Matches the paper's Thm C.2 exactly. | `~/repos/c10-eufcma-port` (out-of-tree) `drafts/WOTS_C_EmbDischarge.ec:173 D1_MEUFNACMA_WOTSC_MM45_embthfc`; `AXIOM_STATUS.json` A5-EUFCMA `discharge_artifacts[type=partial-mechanization]` | **partial** — conditional on `emb_disj_wgpidxs` (FLAG-2). A5-EUFCMA stays `cited-tcb`; the SPHINCS+C composition (`hfx`) and the FORS+C leg are **assumed/open**. Verify gate: compile EVERY `.ec` as a target — `require` does NOT re-verify (proven 2026-07-09) |
| EasyCrypt (A5, FORS+C leg) | ⚠ **The published SPHINCS+C paper has NO FORS+C security theorem** (IEEE S&P 2023 §IV: *"we can use the previous ITSR analysis"*; §V: *"straightforward"*) — a one-paragraph informal `DarkSide_γ` argument, and its Thm 5.2 keeps the **plain** `InSec^itsr(Hmsg)`. Our bespoke `ITSR(+C)` game is therefore **original work**, not a port. | `docs/verification/easycrypt-euf-cma-port-feasibility-2026-07.md` §UPDATE 2026-07-09; `AXIOM_STATUS.json` A5-ITSR scope gap | **open (literature gap)** — A5-ITSR cites Barbosa et al. 2024, a proof of *standard* SPHINCS+. Blocked on: mechanizing the ITSR(+C) reduction |
