...
> `log2_w=3`. UPDATE 2026-07-19: the EasyCrypt reproducibility defect is closed — the container gate (`make -C contracts/verification verify-easycrypt-docker`) compiled the full 21-file closure as targets with zero skips and three fired mutation controls on 2026-07-17 (receipt `contracts/verification/easycrypt/docker/GATE-RECEIPT-2026-07-17.log`); the C10-parameter (log2_w=3) and conditional-capstone residuals remain research, tracked in the c10-eufcma-port feasibility doc. (The census quoted below has since moved to 163 harnesses / 26 files, 156/21 mutation-enrolled, 43 groups — see §C.) The refreshed source census is 162 Kani harnesses/26 files;
