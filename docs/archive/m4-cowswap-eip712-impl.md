# ARCHIVED — M4 Groth16 CowSwap implementation snapshot

> Retired 2026-06-30. This document is provenance only. Do not implement its
> proof, readable-string, VK, command, or payload instructions. See
> [`zk-clear-sign-retirement.md`](zk-clear-sign-retirement.md) and the live
> native guide at
> [`../companion/companion-safe-cowswap-presign.md`](../companion/companion-safe-cowswap-presign.md).

# M4 implementation notes — CowSwap EIP-712 GPv2Order clear-signing

> Companion to [`docs/archive/m4-cowswap-eip712.md`](../archive/m4-cowswap-eip712.md), the
...
- A new firmware module `secure/src/tx/eip712.rs` that turns the
  164-byte canonical buffer into the EIP-712 keccak digest the SPHINCS+C10
  signer consumes.
...
- `docs/companion/m4-cowswap-eip712-impl.md` — this file.
...
- `docs/architecture/architecture.md` — updated the ZK Clear Signing intro to
  mention EIP-712, added a payload-shape table, extended the e2e
  scenarios table.
- `docs/archive/m4-cowswap-eip712.md` — marked as historical with a pointer
...
2. **Receiver, feeAmount, and the balance enums are bound but not
   ...
   decode more.
3. **SPHINCS+C10 signatures, not ECDSA.** The wallet still produces a
   4008-byte SPHINCS+C10 signature over the EIP-712 digest, NOT an ECDSA
