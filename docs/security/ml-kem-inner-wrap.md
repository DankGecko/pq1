...
| 2b | Firmware wiring — add `pqsigner-pq-seal` to `secure/`, derive `seed` (64 B, two `hw::huk` labels) + `huk_secret` (a third label), draw `m` from the TRNG, **allocate MCU-flash `ct` slots** (1568 B × half × account), `seal_half` at provision + `open_half` at reconstruct (`secure/src/dual_se.rs` + the OPTIGA/SE050 store paths now writing the 48 B `aead`) | ⏳ next (flash-layout + driver rewire) |
