...
| 2b-2 | **Provision/reconstruct rewire** — `seal_half_hw` at provision + `open_half_hw` at reconstruct, the OPTIGA/SE050 store paths writing the 48 B `aead`, the `ct` → MCU **non-secure** flash slots (option B). **Open:** a QEMU-testable device-key path (`hw::secret_keys` is `stm32u585`-only today) for the e2e tests; and the real flash-delta number via a bench `make` build (the standalone `stm32u585` secure build hits a *pre-existing* `usb_hw` cfg gap — needs the full `RUSTFLAGS_SECURE_HW` link env) | ⏳ next |
