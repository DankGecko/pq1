...
|---|---|---|---|---|---|
| UC1 | **Frame-length not bounded before a copy** | an attacker USB frame overruns the reassembly / TX buffer | **DEFENDED-by-construction, one candidate gap (2026-07-17 sweep).** Every copy is preceded by a bounds check + a `fi_min` FI clamp (defends the O'Flynn EMFI-on-`min` class): first/continuation frame (`apdu_framing.rs:379-409`), TX (`transport.rs:152`), chunked response (`commands.rs:791`); `MAX_APDU_RX=4096`; `checked_add` re-check even after the clamp. **Candidate (see sca-fi FI10):** `fi_min`'s recompute is algebraically dead and LLVM emits a bare `umin` — the *doubled* `apdu_framing` sites degrade to nested `umin` (still 2-fault), but the single-clamp sites (`transport.rs:159-160`, `commands.rs:791-793`) lose the FI guard entirely | proptest fuzz `apdu_framing.rs:492-594` (`hid_frame_random_input_never_panics`, `parse_apdu_header_never_panics`, `hid_frame_oversize_n_dropped`) | ✅ proptest fuzz / ⚠ FI10 candidate |
...
| UC3 | **APDU-chaining / channel confusion** | a cross-tab confused-deputy interleaves frames, or an INS-swap mid-chain smuggles bytes | **DEFENDED.** Channel-ID isolation (`apdu_framing.rs:386`, regression-pinned `:804-881`); seq monotonicity; INS-swap-mid-chain → reset (`:182-186`); a fresh seq=0 interrupting a partial reassembly aborts + scrubs (`:350-357`); INS allowlist *before* `chain.step` accepts bytes so a bogus INS can't burn 8 KB (`commands.rs:322-330`, `per_cmd_chain_bound:99-112`). **Gap (see UC7):** a *same*-INS retransmitted chunk appends at the stale cursor (no block counter in P1) — dup-append confusion, DoS-class | proptest `chain_step_sequence_pos_within_capacity`; channel-isolation regression tests | ✅ proptest + regression |
| UC4 | **Idle / stalled-reassembly secret residue** | a host declares a large APDU then stalls, leaving a half-assembled buffer | **DEFENDED at two of three layers.** Reassembly 5 s timeout scrubs a half-assembled buffer (`transport.rs:123-136`); GET_RESPONSE 30 s inter-chunk drain timeout (`commands.rs:825-846`); pending-buffer scrub on any non-GET_RESPONSE while a chunked drain is live (`commands.rs:243-247`). **The third layer is uncovered — see UC7** (the ISO-chained-command state has no idle timeout and pins the F11 router lease). Detection note: no host-side timeout tests exist (nonsecure isn't host-compilable) — the "timeout unit tests" claim this row used to carry is unverifiable | NS is untrusted so this is defense-in-depth | ⚠ (defense-in-depth; UC7 gap) |
| UC7 | **Chained-command stall pins the F11 router lease** | one chained APDU opens the single-session lease; nothing resets it — every other channel is rejected forever, across replugs | **CANDIDATE (2026-07-17 sweep, pre-adjudication; persistent DoS).** `ChainState::reset` runs only inside `step` (`apdu_framing.rs:158-161,179-206` — grep: no other caller); the lease re-derives from `chain.active_ins() != 0` (`commands.rs:213-217`); the poll loop checks only the HID assembler and the GET_RESPONSE drain (`main.rs:205-208`). A crashed/malicious first chain-opener (the reference client hardcodes `channelId = 0x0101`) locks out every other client until power-cycle; the IWDG never fires (NS keeps bumping the heartbeat). Fix: idle-frame timeout on chain state + lease release, or reset on USB bus-reset. Companion finding: same-INS retransmit dup-appends (UC3 note) | Host PoC: open a chain on 0x0101, go silent, observe `SW_CONDITIONS_NOT_SATISFIED` on all other channels | ❌ found-this-surface (candidate) |
| UC8 | **Status byte that conveys nothing presented as authoritative** | GET_STATUS's `provisioned` byte is constant-1 (`remaining ≤ MAX_ATTEMPTS` on every path) | **CANDIDATE (2026-07-17 sweep, pre-adjudication; LOW/informational).** `commands.rs:427` derives `provisioned` from `cmd_get_remaining`, which is always ≤ 10 — so the bit is 1 even on a blank device; a companion gating setup-vs-unlock UX on it is misled. Fix: source it from a real provisioning-state veneer, or delete the byte | Host PoC: GET_STATUS on an unprovisioned device | ❌ found-this-surface (candidate) |
| UC5 | **DoS via USB-C warm-reset / hung loop** | a host wedges the transport or PRL so the device stops responding | **DEFENDED (DoS-only).** IWDG hang-reset (NS heartbeat bump per loop `main.rs:195-199` → `nsc_register_heartbeat` → secure SysTick stops feeding IWDG on a hung loop); OTG hardening forces `FDMOD=1` + masks the SOF timing side-channel (`nonsecure/src/usb/mod.rs:121` `harden_otg` — note: it lives in the NS crate, not `secure/src/hw/usb_hw.rs` as previously cited); documented USB-C warm-reset recipes (`cc_open_then_reset`) | `fwup-transport-hw-iwdg` HW e2e (wipe-trigger + warm-reset validation) | ✅ HW e2e |
