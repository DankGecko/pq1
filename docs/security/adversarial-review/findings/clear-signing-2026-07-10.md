...
### F30 — Live companion docs specified the retired Groth16 CoW wire

- **Status:** ✅ FIXED
- **Mode / severity:** integration / availability · MED
- **Location:** `docs/companion/companion-safe-cowswap-presign.md`; archived M4 notes and Safe references
- **What:** active instructions still described proofs, readable strings, VK injection, AddrOnly/proof modes, and obsolete payload sizes, causing a conforming companion to send a wire format current firmware refuses.
- **Disposition:** CONFIRMED_REAL
- **Resolution:** the live guide now specifies native `canonical[204]` plus optional per-leg ERC-20 bundles (`204..=2448` bytes), native digest binding, and exact Safe/MultiSend rules. Historical M4 material is quarantined under `docs/archive`; frozen kind-3 naming is explicitly compatibility-only.
