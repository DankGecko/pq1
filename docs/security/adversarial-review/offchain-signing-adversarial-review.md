...
|---|---|---|---|---|---|
| OC1 | **Companion pre-nests a hash → UserOp forgery oracle** | firmware bare-signs a companion-chosen 32-byte value that equals a `sphincsDigest` | **DEFENDED (the historic HIGH, closed for ALL kinds).** Firmware always calls `aa::eip1271::replay_safe_hash` (`eip1271.rs:163-181`): RAW32 (`cmd_sign_offchain.rs:699`), PERSONAL_SIGN (`:702`), EIP712_TYPED / _V3 (`:572`). For V3 the `nested_blob` is **display-only** — the signed digest is byte-identical to kind=2 (`:434-458`) | Regression `aa/tests/offchain_raw32_forgery.rs` (feeds a real drain `sphincsDigest` as the raw32 payload, asserts `replay_safe_hash(H) != H`); unit `eip1271.rs:317-336` | ✅ regression + unit |
...
| OC3 | **Counter compaction loses / duplicates an increment** | a fault or power-loss during page-123 compaction rolls the count back | **DEFENDED (with a disclosed bound).** `compact_page` fails closed on >256 slots *before* erasing (`flash.rs:1546-1548`); replay order (USEROP_SIGS → USEROP → COUNT per slot) makes a torn compaction unable to roll a *registered* slot's few-time tally to 0 (`:1557-1610`). **Residual (disclosed `:1578-1580`)**: a torn COUNT/USEROP rollback is still possible but bounded by on-chain `_setOffchainSigCount` monotonicity + gap ≤ 100 + on-chain `slotUses`. Reads F-12 hardened (fwd+rev double scan, halt on mismatch) | Rainbow `fault_sweep_flashctr.py` (770 rollbacks pre-fix → ~10 residual post-fix); aim FI at the erase-then-replay window | ✅ FI sweep (residual bounded) |
| OC4 | **Gap check bypass** | more than `MAX_OFFCHAIN_GAP` unbacked off-chain sigs slip through | **DEFENDED.** `gap = local_offchain.saturating_sub(last_userop); if gap >= 100 { OffchainGapExceeded }` (`cmd_sign_offchain.rs:331-335`), with double-read halt-on-mismatch inputs (`:287-303`) and an F-10 recheck after another `wait_random` (`:349-356`) | Rainbow `fault_sweep_cap.py` (mirrors the gap+cap gates verbatim) | ✅ FI sweep |
| OC5 | **Combined-cap overflow** | `slotUses + offchainSigCount` exceeds the cap via wrap or a stale read | **DEFENDED.** `checked_add(1)` (`:336-339`) + `saturating_add` cap check (`:343-346`) + `OFFCHAIN_COUNT_CEILING = MAX_SLOT_USES-1` funnels every durable writer (`offchain_state.rs:104-121`); on-chain mirror `PQSmartWallet.sol:479`. **Note to probe**: the `userop_sigs` double-read (`:322-329`) does *not* reject `== u64::MAX` (unlike the other two reads); it is fail-closed only because a glitched max saturates the cap — confirm no path uses it un-saturated | `fault_sweep_cap.py`; the xtask drift gate (`xtask/src/main.rs:786` fails on `MAX_OFFCHAIN_GAP` drift) | ✅ FI sweep + drift gate |
| OC6 | **Unregistered-slot off-chain sig accepted post-restore** | after a restore, a slot with no on-chain registration gets an off-chain sig | **DEFENDED.** `if !offchain_count_is_registered(slot_key)` only the ERC-6492 counterfactual (`!account_deployed && slot_index==0`) may proceed; every other unregistered slot → `OffchainSlotUnregistered` (`:266-276`), forcing a Type-1 rotation via `CMD_SIGN_USEROP` first | Probe an FI flip of `offchain_count_is_registered` (mitigated by the F-12 double scan `flash.rs:1888`) | ✅ (F-12 + gate) |
| OC7 | **ERC-6492 `slot_index != 0` accepted** | the counterfactual path signs for a slot the factory never seeds | **DEFENDED.** Rejected twice: early `!account_deployed && slot_index != 0` (`:178-181`) and inside the unregistered probe (`:272`); the larger 8616-B write is re-validated inside the counterfactual branch (`:185-191,:917-923`) to bind it to the larger extent (guards a single-fault deployed→counterfactual flip) | Probe a single-fault flip of `slot_index` / `account_deployed` between the two checks | ✅ (double-checked) |
| OC8 | **Counterfactual sign without auto-register** | a never-deployed wallet gets an off-chain sig with no slot-0 registration | **DEFENDED.** Slot-0 auto-registration is the post-confirm durable bump (`:880`); MEDIUM-3 explicitly *removed* the early pre-write (`:249-255`) so no durable state lands before the confirm | Verify no early durable write exists before the confirm | ✅ (MEDIUM-3 fix) |
...
5. **FI sweeps + drift gate.** `fault_sweep_cap.py` (gap+cap mirror), `fault_sweep_flashctr.py` (counter rollback); `xtask` fails the build on `MAX_OFFCHAIN_GAP` drift. Frozen-format tests pin `EIP6492_MAGIC` + `MAX_OFFCHAIN_GAP`. **Gap**: no Kani harness covers `offchain_state` (Kani is only in `aa/userop.rs` + `ns_ptr_validate.rs`) — a bounded-verification opportunity.
...
RULES:
  ...
  - For each finding: OC-mode, file:line, PoC, disposition, severity, proposed fix (flag if it
...
**Running it as a swarm.** ≥3 reviewers per scope, cross-vote, two model backends. Split the four kinds one-per-reviewer for OC1 (each proves its kind nests).
