...
*Affects Claim 2 against any device that has not been hand-rotated.* Until the first-boot self-provisioning flow lands (work-todo #36 — devices ship at RDP-0 with factory-installed *transport* keysets, self-lock to RDP-2 on first field boot, then rotate SCP03/PBS on-device with fresh TRNG salt; supersedes the fixture-side stage 2 of `docs/security/production-security.md §2.2`), un-rotated bench boards use the NXP factory-default SCP03 keys (published in AN12436) for SE050 SCP03 and a development OPTIGA PBS. **Pre-production only.** No shipping device passes QA without the factory transport-keyset install, and no device is user-ready until its first-boot rotation completes.
