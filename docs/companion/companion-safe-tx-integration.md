...
| `[112]`     | `operation`       | u8              | `0 = Call`, `1 = DelegateCall`. **`1` accepted only for an allowlisted MultiSend batch**, see below |
...
6. **Operation gate**: `canonical.operation == 0` (Call) always; DelegateCall
   (`1`) is accepted **only** for an allowlisted `MultiSendCallOnly` batch
   (pinned target + `multiSend` selector, strict per-record decoding) —
   any other `operation == 1` is **rejected**. See
   [DelegateCall and MultiSend](#delegatecall-and-multisend) below.
...
`canonical.operation == 1` (DelegateCall) is accepted **only** when the
inner call is a `multiSend` batch (selector `0x8d80ff0a`) to one of the
pinned canonical `MultiSendCallOnly` deployments — the exact shape the
Safe Transaction Service returns when the user batches an op in the Safe
UI (owner add+threshold change, multiple token transfers in one SafeTx,
the `[approve, setPreSignature]` CoW shape, etc.). The packed records
are strictly decoded (per-record `operation == 0`, record cap, canonical
ABI framing) and each routes through the same inner classify ladder with
divider pages. Any other `operation == 1` — a DelegateCall to a target
not on the allowlist — is still **rejected at the verifier**; a
DELEGATECALL is never blind-signed.

The full wire shape, allowlist, hard decode rules, refusal banners, and
companion checklist for MultiSend are documented in
[`companion-safe-cowswap-presign.md`](companion-safe-cowswap-presign.md#multisend-batches-the-shape-the-safe-ui-actually-emits)
(the multiSend section applies to all batch flavours, not just CoW).

Your obligations as a companion:

1. **Use a canonical `MultiSendCallOnly` deployment** as the SafeTx `to`.
   Plain `MultiSend` (delegatecall-capable records) and non-allowlisted
   targets are refused with `Safe sign / safe_v1 required`.
2. **Forward the original MultiSend payload unmodified** — `raw_data` is
   the full `multiSend(bytes)` calldata, and `canonical.data_hash` must
   be `keccak256` of it.
...
async function buildSignRequest(safeAddress, safeTxHash, signer) {
  ...
  if (tx.operation !== 0) {
    // This minimal example only covers single-call (operation == 0) SafeTxs.
    // MultiSend batches (operation == 1, MultiSendCallOnly target) ARE
    // supported by the firmware — see companion-safe-cowswap-presign.md
    // for the batch wire shape; handle them on the dedicated path instead.
    throw new Error("MultiSend batch — use the MultiSend flow, not this single-call path.");
  }
