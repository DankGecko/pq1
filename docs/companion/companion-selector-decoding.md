...
- Slots are **positional**. To say "no ERC-20 metadata, no retired v1 payload,
  no native CoW v3 trailer, no Safe, no curated selector, but yes
  self-attest", write `0x0000` for the first five lengths, then
  `[self_attest_len][self_attest_bundle]`.
...
- **Phase 3 per-contract attestation** — the eventual fourth Merkle
  ...
  `docs/archive/calldata-decoding-handoff.md § Migration to ERC-7730`.
...
- **Batch sign** — `CMD_SIGN_USEROP_BATCH` (cmd 30) carries one
  ...
  `docs/companion/companion-batch-sign-integration.md`.
