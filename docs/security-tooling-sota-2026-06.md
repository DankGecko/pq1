...
|---|---|---|
| **`cargo-checkct`** (Ledger Donjon) | **Proves no_std `thumbv8m` secret paths compile to constant-time machine code** — closes the gap `subtle` + source review leaves after LTO/`opt-level=s`. thumbv8m explicitly supported — and the **only** tool that proves CT on the shipped M33 ISA (Binsec/Rel has no ARMv8-M decoder; see §4). The single best find of the sweep. | [validated] |
