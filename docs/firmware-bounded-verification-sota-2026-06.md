...
| Kani | (2) counter *gate* + policy | **DONE 2026-07-03** (work-todo §12e) | `check_offchain_gate` extracted to `aa/src/offchain_gate.rs`, **used in place** by `cmd_sign_offchain`; 8 harnesses — single-gate soundness + verdict-exact control + sequence/interleave (2-step gap+cap limit-slicing, monotonicity, slot isolation) + both bricks unreachable (value-inflation + distinct-slot); 3 `kani_mutations.json` anti-vacuity entries; differential host test binds the model to the shipped mock | done | high | proves the counter *policy* over adversarial op-orderings; still **misses the torn-compaction rollback HIGH** — below the seam in `flash.rs::compact_page`, needs a crash model (Verus row) / stays on the `fault_sweep_*.py` sweeps |
