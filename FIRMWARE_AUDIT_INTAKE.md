...
6. **Trusted-display clear-signing integrity.** The human-readable intent shown on the NV3007 LCD and the hash actually signed must derive from the **same S-stack copy** — the companion never gets to substitute a digest. Covers the native ERC-20 / Safe `SafeTx` / CowSwap `GPv2Order` / ERC-7730 decoders. A decode-vs-sign mismatch = user confirms X, signs Y. The outer UserOp sender is now hard-bound to `GET_WALLET_ADDRESS(account_index)` and only the derived address reaches verifiers/hashes. **Open display residual:** the account index/source address is not rendered, so a companion can still choose a different valid one of the user's 256 accounts behind otherwise identical pages; multi-account production UX should surface the source identity.
