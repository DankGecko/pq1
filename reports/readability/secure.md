...
- **Cargo.toml comment churn.** Roughly half of `secure/Cargo.toml` is feature-flag documentation written like commit messages ("Phase-8 PR 1 — axis aliases", "work-todo #20 Stage A"). All useful, but rotting on a per-PR cadence. A pass that collapses these into a single `docs/firmware/feature-flags.md` and shrinks the Cargo.toml comments to a one-liner per flag would help — too large for this audit.
