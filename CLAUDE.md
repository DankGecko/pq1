...
3. **E2E encrypted SE tunnels.** OPTIGA Shielded Connection (TLS-PRF + AES-128-CCM-8, PBS **DHUK-derived at boot** via `hw::secret_keys::derive_into("pqsigner/optiga-pbs-v1")` — no longer flash-page-126-sealed; page 126 was freed by work-todo #24 and now holds the fw-update verify-failure counter + the bank-1 wrapped SE050 BHK). SE050 SCP03 (AES-CMAC + AES-CBC). No plaintext secret on I2C. ML-KEM-1024 inner wrap planned.
