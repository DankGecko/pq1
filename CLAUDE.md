...
**Trusted-display clear-signing.** Every signable artifact is decoded and rendered inside the secure world before the user presses confirm — no blind-sign path for known shapes. (1) **Safe transactions:** the EIP-712 `SafeTx` typed-data hash is verified in S-world (`secure/src/tx/eip712/safe/`) and the inner `to/value/data/operation` is decoded locally — ERC-20 transfers and Safe owner/threshold/module/guard changes render on the OLED with full parameters; the companion never gets to substitute a hash. (Safe `multiSend` batches — selector `0x8d80ff0a` — have no on-device decoder and fall, loudly and WYSIWYS-safely, to blind-sign.) (2) **CoW Swap orders:** the EIP-712 `GPv2Order` is verified in S-world (`secure/src/tx/eip712/cowswap/`) and a Groth16 proof over BLS12-381 (`secure/src/zk/`) attests that the order payload matches what the trusted UI displays — the user sees the exact intent (e.g. `SELL 0.2 USDC for at least 0.0004 WETH`) rather than a 32-byte digest. Same verifier path is reused by ERC-7730 clear-sign descriptors and the typed-call ABI parser. (3) **Safe-wrapped CoW orders:** when a SafeTx's inner call is CowSwap `GPv2Settlement.setPreSignature(orderUid, true)`, the same CoW v3 pipeline verifies the order bound to the SafeTx's inner calldata with `orderUid.owner == the Safe` (not the wallet `sender`), and the render combines Safe context (banner, address, nonce, refund pages) with the full order intent — unmistakably "a CoW order for this specific Safe". One binding resolver (`secure/src/tx/eip712/safe/cow_binding.rs`) and the shared `cowswap_display::append_order_body_pages` keep both flows code-identical; see `docs/companion-safe-cowswap-presign.md`.
...
- **⚠️ SHIP BLOCKERS — OPTIGA shipping-state lockdown (S-1, S-2, S-3 — all three required before any device leaves the bench).** Full spec in `docs/production-todo.md` "OPTIGA Trust M V3 — LcsO transitions" and ship-blocker section at top of `docs/work-todo.md`. Briefly:
  - **S-1 — F1D0 `Change = ALW`:** `apdu.rs:930` and `:1059` allow a desoldered-OPTIGA bench attacker to overwrite F1D0 with a chosen HMAC key, self-auth, reset E120, and brute-force PINs without bound. Fix: `Change = Auto(F1D0)` (preserves user PIN-change) + ratchet to LcsO=Op; fallbacks `NEV` / `LcsO<Op`; **`Conf(E140)` is NOT acceptable** (PBS-extraction bypass).
  - **S-2 — Trust anchor at `0xE0E3` is Infineon's PUBLIC sample cert** (`reset.rs:26-49`). The matching private key is publicly available; anyone can sign a `SetObjectProtected` manifest that bypasses every Change AC on every OID, including any S-1 fix. **S-1 alone is insufficient — S-2 must close simultaneously.** Fix: replace with a PQ1-factory-HSM-controlled cert (or remove the TA entirely), lock `0xE0E4..0xE0E8`, lock `0xF1D7` spare, `compile_error!` gate `optiga-reset-oids`.
  - **S-3 — `optiga-hw-counter` must be mandatory for production.** Without it, F1D0.Execute=ALW + a `Conf(E140)`-gated soft counter at F1E1 means a desoldered/PBS-leaking attacker gets unbounded HMAC attempts. Fix: `compile_error!` in `mode-production` when `optiga-hw-counter` is off, plus `compile_error!` on `build_metadata_counter` in production builds.
  - **S-5 — RESOLVED 2026-05-28.** SE050 SCP03 now negotiates `P1=0x33` (C-MAC + C-DEC + R-MAC + R-ENC) and `scp03::unwrap_response` is wired into `send_apdu`. Mirrors NXP plug-and-trust `nxScp03_Com.c:124-248`. AES-128-CBC decrypt round-trip + R-MAC layout test landed in `scp03_logic`. Logic-analyzer verification on a real B-U585I-IOT02A still pending — capture an unlock cycle and confirm response phase is ciphertext + 8-byte R-MAC, no `half_E` plaintext.
