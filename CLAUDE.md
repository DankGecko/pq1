...
- **⚠️ SHIP BLOCKERS — OPTIGA shipping-state lockdown (S-1, S-2, S-3 — all three required before any device leaves the bench).** Full spec in `docs/production-todo.md` "OPTIGA Trust M V3 — LcsO transitions" and ship-blocker section at top of `docs/work-todo.md`. Briefly:
  ...
  - **S-5 — SE050 SCP03 at P1=0x03 leaks `half_E` on I²C bus on every read.** `scp03.rs:213` negotiates C-MAC + C-DEC only; `s_rmac` is derived (`:194`) but never used; `send_apdu` (`se050/apdu.rs:262-282`) reads SW + TLV directly off cleartext. **Fires on a working device every unlock — no desoldering required.** Fix: switch EXTERNAL_AUTHENTICATE to P1=0x33 (adds R-MAC + R-ENC) and implement `scp03::unwrap_response` mirrored on OPTIGA's `shield.unwrap_response`. Verify on logic analyzer that response bytes are ciphertext post-fix.
  - **S-6 — SE050 admin-delete entry on USERID_OBJ enables seed theft via PIN substitution.** `mod.rs:1598-1604` passes `Some(ADMIN_WIPE_OBJ)` for the UserID's admin policy entry. An admin-credential compromise → delete USERID_OBJ → recreate with attacker's PIN at the same ID → ENTROPY_OBJ's policy is satisfied → `half_E` exfiltrated. Policies are immutable post-creation; fix requires changing provisioning AND wiping every existing chip. Fix: pass `None` for the UserID's admin entry (keep it on the data objects so admin can still DoS-wipe); replace the lockout-cleanup path with SE050 full-chip `factory_reset`.
  - Also tracked in `docs/security-review-2026-05.md` §§C-4/C-5/C-6/C-7/C-8 and `docs/threat-model.md` Claim 3 (provisional flag). The bring-up state is acceptable ONLY because no devices have shipped.
