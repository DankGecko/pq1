...
/// Compute a combined Miller loop for exactly 4 (G1, G2) pairs, returning a
/// single `MillerLoopResult` that can be finalized with `.final_exponentiation()`.
///
/// This avoids 4 separate final exponentiations (the expensive part), cutting
/// total pairing cost roughly in half. No heap allocation required — the pair
/// count is compile-time fixed.
///
/// Used by the Groth16 verifier:
///   `miller_loop_4([(A,B), (-α,β), (-vk_x,γ), (-C,δ)]).final_exponentiation() == Gt::identity()`
#[cfg_attr(docsrs, doc(cfg(feature = "pairings")))]
pub fn miller_loop_4(pairs: [(&G1Affine, &G2Affine); 4]) -> MillerLoopResult {
    struct MultiAdder {
        cur: [G2Projective; 4],
        base: [G2Affine; 4],
        p: [G1Affine; 4],
    ...
    impl MillerLoopDriver for MultiAdder {
        type Output = Fp12;

        fn doubling_step(&mut self, f: Self::Output) -> Self::Output {
            let mut acc = f;
            for i in 0..4 {
                let coeffs = doubling_step(&mut self.cur[i]);
                acc = ell(acc, &coeffs, &self.p[i]);
            }
            acc
        }
        fn addition_step(&mut self, f: Self::Output) -> Self::Output {
            let mut acc = f;
            for i in 0..4 {
                let coeffs = addition_step(&mut self.cur[i], &self.base[i]);
                acc = ell(acc, &coeffs, &self.p[i]);
            }
            acc
        }
    ...
    let mut adder = MultiAdder {
        cur: [
            G2Projective::from(qs[0]),
            G2Projective::from(qs[1]),
            G2Projective::from(qs[2]),
            G2Projective::from(qs[3]),
        ],
        base: qs,
        p: ps,
    };
