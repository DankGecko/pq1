//! Heavy wordlist code — `Mnemonic` codec, the 8-byte flat wordlist,
//! constant-time exact / prefix lookups, PBKDF2-HMAC-SHA512 seed
//! derivation.
//!
//! Gated behind the `full-wordlist` Cargo feature so the FSBL can omit
//! ~18 KB of rodata it doesn't need (full `WORDLIST` + `WORDLIST_FLAT`
...
use crate::{read_11_bits, BITS_PER_WORD, ENTROPY_BYTES, MAX_WORD_BYTES, SEED_BYTES, WORD_COUNT};

// `wordlist` is a top-level module on `crate` (feature-gated there too),
// so the existing on-disk layout `bip39/src/wordlist.rs` is preserved.
// Re-export `WORDLIST` from this module so `pub use full::*` in `lib.rs`
// keeps the historical `sphincs_tz_bip39::WORDLIST` public path.
