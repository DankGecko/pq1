...
const PROD_ROOT_HEX: &str = "48aac419f76889375e21dd7813b0abefafa02485a5d1f52e510d854d97192e96";
...
fn upstream_fixture_corpus_is_exact_test_only_and_honestly_inventoried() {
    ...
    let accepted: BTreeSet<PathBuf> = registry
        ...
        .collect();
    assert_eq!(accepted.len(), 235);
    assert_eq!(accepted.intersection(&tested_descriptors).count(), 150);
    assert_eq!(accepted.difference(&tested_descriptors).count(), 85);
    assert_eq!(tested_descriptors.difference(&accepted).count(), 122);
}
