//! Offline evidence and renderer coverage for the bounded Safe core slice in #497.
//!
//! This test admits only three operand-complete direct Safe singleton calls.
//! SafeL2, migrations, proxy instances, module routes, fallback authority,
...
const EVIDENCE_PATH: &str = "tests/erc7730-semantic-evidence/safe-core";
...
const V130_CANONICAL: &str = "0xd9Db270c1B5E3Bd161E8c8503c55cEABeE709552";
const V130_EIP155: &str = "0x69f4D1788e39c87893C980c06EdF4b7f686e2938";
const V141: &str = "0x41675C099F32341bf84BFc5382aF534df5C7461a";
const V150: &str = "0xFf51A5898e281Db6DfC7855790607438dF2ca44b";
...
fn assert_source_semantics(main: &str, owners: &str, version: &str) {
    assert!(
...
#[test]
fn safe_core_offline_evidence_is_complete_and_consistent() {
    let manifest = assert_receipted_package(23);
    assert_eq!(
    ...
    let runtime_130 = read_hex(&evidence().join("runtime/Safe-1.3.0.ethereum.hex"));
    let runtime_141 = read_hex(&evidence().join("runtime/Safe-1.4.1.ethereum.hex"));
    let runtime_150 = read_hex(&evidence().join("runtime/Safe-1.5.0.ethereum.hex"));
...
#[test]
fn safe_core_descriptors_pin_official_deployments_and_refusal_boundary() {
    let versions = [
        (
            "calldata-Safe-1.3.0.json",
            "deployments/v1.3.0-gnosis_safe.json",
            14,
        ),
        (
            "calldata-Safe-1.4.1.json",
            "deployments/v1.4.1-safe.json",
            7,
        ),
        (
            "calldata-Safe-1.5.0.json",
            "deployments/v1.5.0-safe.json",
            2,
        ),
    ];
...
#[test]
fn safe_core_catalogue_merkle_binds_three_formats_and_refuses_known_siblings() {
    ...
    let source_counts = [
        ("calldata-Safe-1.3.0.json", 14),
        ("calldata-Safe-1.4.1.json", 7),
        ("calldata-Safe-1.5.0.json", 2),
    ];
