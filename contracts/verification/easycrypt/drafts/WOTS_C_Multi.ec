  ...
  proc choose() : qC list = {
    ...
    while (k < size qs) {
      ...
      (* The WOTS-TW committed message for instance k IS the +C digest d
         (= ThC pp wad m sd); msgWOTS = dgstblock (WOTS_TW_ES.ec:219), so d is
...
proof.
  ...
  + sp.
    ...
    + move=> &1 z; wp; skip => /> &2 *; smt(take_nth map_rcons grindC_E size_ge0).
    skip => /> *; smt(take_size size_ge0).
  sp.
  seq 1 1 : (={glob A, glob STCRC_WC.Col.O_THFC_Default} /\ ps{1} = pp{2}
             /\ pp0{2} = pp{2}
