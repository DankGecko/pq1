   ...
   *scheme module* in the repo (building it + re-running MM45's whole section
   ...
   NOT in the closure (checked, worth stating because they are the App-D gap):
     * grindCP  ("+C grinding always finds a good counter" = App-D gap #1 for
         WOTS+C) is ABSENT.  It lived in WOTS_C_Encoding.ec, which was DELETED
         2026-07-09 (broken, orphaned).  D1's grinding is modelled operationally
         via STCRC_WC.G.grind, not axiomatised.
...
require FORS_C_Multi.
clone FORS_C_Multi.MFORSC as M.
...
lemma EUFCMA_SPHINCS_PLUS_C
  ...
  (A_fors <: M.Adv_EUFCMA_MFORSC{-M.R_ITSRC_MFORSC, -M.O_CMA_MFORSC,
                                      -M.O_CMA_MFORSC_I, -M.F.O_ITSRC_Default,
                                      -M.EUF_CMA_MFORSC_I})
  ...
  &m :
    ...
    (* FLAG-2 discharged in-stack (rebase 2026-07-09) -- no longer a premise. *)
    (forall (mk : M.F.mkey) (m : M.F.msg),
       exists (cf : M.F.cntr), M.F.predC_fors (M.F.mco mk m cf)) =>
    ...
    (   Pr[M.EUF_CMA_MFORSC_I(A_fors).main() @ &m
             : res /\ !M.EUF_CMA_MFORSC_I.covered]
    ...
    p_sphincs_c
      <= skg_adv
       + mkg_adv
       + Pr[M.EUF_CMA_MFORSC(A_fors, M.O_CMA_MFORSC).main() @ &m : res]
       + p_xmssmt_c =>
    ...
    p_sphincs_c
      <= skg_adv
       + mkg_adv
       (* +C substitution #1: FORS+C multi expansion (EUFCMA_MFORSC) *)
       + ( Pr[M.F.ITSRC(M.R_ITSRC_MFORSC(A_fors),
                           M.F.O_ITSRC_Default).main() @ &m : res]
           + mtree_openpre + mtree_trh + mtree_trco )
       ...
       + ( Pr[STCRC_WC.S_TCR_C(R_multi_STCRC(A_wots),
           ...
           + xmssmt_trees ).
proof.
  move=> hc encb hgc htree hfx hbridge.
  ...
  have hF := M.EUFCMA_MFORSC A_fors mtree_openpre mtree_trh mtree_trco &m hgc htree.
