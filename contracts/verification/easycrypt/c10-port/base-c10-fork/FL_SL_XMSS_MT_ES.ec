...
(* _UG twin of the helper above: same statement and same proof, about the
   UNGATED game.  The restored MM45 hypertree proof uses this exactly once. *)
local lemma EqPr_MEUFGCMAWOTSTWESNPRF_Orig_V_UG &m :
  Pr[M_EUF_GCMA_WOTSTWESNPRF_UG(R_MEUFGCMAWOTSTWESNPRF_EUFNAGCMA(A), O_MEUFGCMA_WOTSTWESNPRF, FC.O_THFC_Default).main() @ &m : res]
  =
  Pr[M_EUF_GCMA_WOTSTWESNPRF_UG(R_MEUFGCMAWOTSTWESNPRF_EUFNAGCMA(A), O_MEUFGCMA_WOTSTWESNPRF_V, FC.O_THFC_Default).main() @ &m : res].
proof.
byequiv => //.
proc.
seq 4 4 : (={glob A, glob R_MEUFGCMAWOTSTWESNPRF_EUFNAGCMA, ps, O_MEUFGCMA_WOTSTWESNPRF.qs, FC.O_THFC_Default.tws}); 2: by sim.
...
+ rewrite EqPr_MEUFGCMAWOTSTWESNPRF_Orig_V_UG.
  byequiv=> //.
  proc.
...
move: (MEUFGCMA_WOTSTWESNPRF (R_MEUFGCMAWOTSTWESNPRF_EUFNAGCMA(A)) _ _ &m)
      (* ===================================================================== *)
      (* THE RESIDUAL, AND IT IS NOW *ONLY* THIS COMPOSITION STEP.             *)
      (* MM45's 2,202-line hypertree proof above is RESTORED AND CHECKED       *)
      (* (about the ungated twin `M_EUF_GCMA_WOTSTWESNPRF_UG`).  What is NOT   *)
      (* available in the fork is the other input to this `smt()`:             *)
      (*                                                                       *)
      (*   restored lemma : Pr[NAGCMA] <= Pr[UG]   + PKCO + TRH                *)
      (*   MEUFGCMA_..    : Pr[GATED]  <= UD + TCR + PRE                       *)
      (*                                                                       *)
      (* To conclude we need `Pr[UG] <= UD + TCR + PRE`, i.e. the UNGATED       *)
      (* security theorem.  The fork proves only the GATED one, and the        *)
      (* available inequality runs the WRONG WAY: the gated success condition  *)
      (* is STRICTLY STRONGER, so Pr[GATED] <= Pr[UG], never the reverse.      *)
      (*                                                                      *)
      (* Closing this needs MM45's ungated WOTS-TW security development        *)
      (* alongside the gated one -- which re-introduces the unrelativized      *)
      (* `two_encodings` axiom the fork exists to retire.  OPEN DECISION.      *)
      (*                                                                      *)
      (* SCOPE OF THE TAINT IS UNCHANGED by this restoration: this lemma and   *)
      (* SPHINCS_PLUS.EUFCMA_SPHINCS_PLUS remain admit-backed.  What changed   *)
      (* is WHAT is admitted -- a 1-line composition instead of a 2,202-line   *)
      (* proof.  Do NOT read this as "the fork is admit-free".                 *)
      (* ===================================================================== *)
      (EUFNAGCMA_FLSLXMSSMTTWESNPRF_MEUFGCMAWOTSTWES &m allnchads allnpkcoads allntrhads); 3: admit.
+ move=> O OC Oll OCll.
  proc; inline *.
  wp.
