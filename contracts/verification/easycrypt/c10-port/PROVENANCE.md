...
⚠ **PARAMETER QUALIFIER — read `UPDATE 2026-07-25 — THE DEPLOYED-PARAMETER BOUNDARY` at the
bottom before quoting ANY result in this file.** Every EUF-CMA / capstone claim below holds at
**MM45-admissible WOTS parameters (`w ∈ {4,16,256}`)**, NOT at deployed C10 (`W=8, L=43,
TARGET_SUM=205`). No part of this development is instantiated at deployed C10.
...
### UPDATE 2026-07-25 — THE DEPLOYED-PARAMETER BOUNDARY (governs every claim in this file)

**Every EUF-CMA / capstone result recorded above holds at MM45-ADMISSIBLE WOTS PARAMETERS
(`w ∈ {4,16,256}`). It is NOT instantiated — and cannot be instantiated — at the DEPLOYED C10
configuration (`W = 8`, `LOG_W = 3`, `L = 43`, `TARGET_SUM = 205`,
`PQSigner_OS/sphincs-c10/src/params.rs:43,46,49,52`).**

Adjudicated 2026-07-25 by three source-level lenses with machine-checked receipts plus two external
adversarial reviewers. Full write-up (mechanisms, DP table, receipts, DO-NOT list):
`PQSigner_OS/docs/verification/easycrypt-euf-cma-port-feasibility-2026-07.md`, section
**UPDATE 2026-07-25d**. In-tree faithfulness header: `drafts/SphincsC10Content.ec:90-197` (commit `f3675aa`).

**The obstruction, in one paragraph.** `const log2_w : { int | log2_w = 2 \/ log2_w = 4 \/ log2_w = 8 }
as val_log2w` (`FV-SPHINCSPLUS-EC/proofs/WOTS_TW_ES.ec:31`) excludes `log2_w = 3`, and it is
*single-sourced* through both clone levels (`FL_SL_XMSS_MT_ES.ec:542,578`; `SPHINCS_PLUS.ec:549,614`),
so the +C stack runs at the same `w` — "it is only the black-box standard-WOTS leg" is unavailable.
`len = len1 + len2` with `1 <= len2` (a **derivable** lemma, `:133`, since `len2` is a *definition* at
`:40`) forces `len > len1`, whereas deployed C10 signs exactly `L = 43 = len1` chains. And decisively:
applied in both argument orders, `axiom two_encodings` (`:572-576`) forces `encode_msgWOTS` to be
**injective with an antichain image**, so `2^128` messages must fit the maximum antichain of
`{0..w-1}^len` — which at `(w=8, len=43)` holds only **2^123.76** points. **The axiom is therefore
UNSATISFIABLE at deployed geometry: no function whatsoever satisfies it.** (Thresholds: `len=45` →
2^129.73 ok; MM45's own shape `len=46` → 2^132.71 ok; SPHINCS+ `w=16, len=35` → 2^133.90 ok.
de Bruijn–Tengbergen–Kruyswijk cited, not formalised; exact DP cross-checked by two reviewers and a
Gaussian approximation.) Hence the three grounds are **one coupled obstruction**, not three independent
ones: relaxing `val_log2w` and `len` is **insufficient**.

**CAN be claimed (unchanged — the artifact is sound):** the +C delta *is* machine-checked; the
SPHINCS+C EUF-CMA reduction is a valid CERTIFIED-0-ADMIT theorem reducing SPHINCS+C EUF-CMA to
{ITSRC10 hardness + the mtree premises + the trusted MM45 base}, **at MM45-admissible WOTS parameters**.
It is **not vacuous and not wrong**: `two_encodings` is satisfiable at every admissible instantiation
(`FV-XMSS-EC/proofs/WOTS_TW_Checksum.ec:312` realizes it), and `w = 8` is unsubstitutable, so the
unsatisfiable regime is unreachable from here.

**CANNOT be claimed:** "SPHINCS+C10 EUF-CMA is machine-checked", unqualified — `val_log2w` is ambient in
every theory requiring `SPHINCS_PLUS`, including the FORS wiring (`GprocFORSC10.ec:53`). In particular
**do not claim the FORS+C10 leg is "proven at deployed FORS geometry"**: the localization result (only
`log2_w` is restricted; `ge1_n`/`ge1_k`/`ge1_a` and `ge1_hp`/`ge1_d` do not exclude deployed
`n=16/k=13/a=11/h'=9/d=2`) says only **where future repair work would live**. Also do not write "the +C
proofs do not depend on `two_encodings`" — EasyCrypt has no `#print axioms`; say they do not **cite** it
and that its consumer chain terminates in a theorem with zero application sites.

**⚠ NOT A DEFECT IN C10.** C10's encoding is *deliberately* non-injective: same-encoding pairs are meant
to exist and merely to be hard to *find*, which is exactly what an S-TCR-on-Th+C term pays for. Deployed
`L = 43` sits two chains below the injective-antichain threshold of 45 at `w = 8` — C10 is operating
precisely in the regime WOTS+C was designed for, which MM45's WOTS-TW interface structurally cannot
express. State this guard **every** time the unsatisfiability is mentioned.

**⚠ THE UNQUALIFIED PHRASING SURVIVES IN THIS REPO'S COMMIT MESSAGES** (e.g. `16255808`: *"reducing
SPHINCS+C10 EUF-CMA to {ITSRC10 hardness + mtree premises + the trusted MM45 base}"*). History is **not**
being rewritten. Where the docs and a commit message disagree, **the docs govern**.

**DO NOT, absent an explicit dated owner decision recorded in THIS file:**
1. **The F1-only repair.** Relaxing `val_log2w` to `1 <= log2_w` and deleting `val_w` (`:61`) / `val_len1`
   (`:96`) is proven cheap, behaviour-preserving and 0-admit across all three vendored levels, and yields
   `len1 = 43` exactly at deployed `n` (receipts: `scratch/f1probe/`, kept deliberately to foreclose
   re-litigation). **It buys ZERO claimable ground** — `len = 46 <> 43`, so F2 and F3 both still block. It
   is the highest-risk wrong move available *because* the receipts look like a green light.
2. **F1+F2 without F3.** That leaves the unconditional `two_encodings` with **no model** at C10 — every
   downstream lemma becomes **vacuously true**. A compiling-but-vacuous artifact is strictly worse than an
   honest gap (trap T3). Gate any such attempt with a satisfiability canary, never with compilation.
3. **Editing or forking the vendored MM45 proof.** No EasyCrypt clone can weaken an inherited axiom **that is NAMED in the clone's `proof`
     clause** (such an override is an `exact` obligation for the same substituted formula), so
     every route to a deployed-parameter claim patches or forks `WOTS_TW_ES.ec`.
     **CORRECTED 2026-08-02 (adversarial review, run 7).** As originally written — "No
     EasyCrypt clone can weaken an inherited axiom" — this was FALSE, and it was load-bearing
     prose inside a DO-NOT list. An axiom OMITTED from the `proof` clause is installed verbatim
     under the substitution with NO obligation, even when the result is false. Demonstrated:
     `abstract theory TT. type t. op c : int. axiom ge0c : 0 <= c. end TT.` then
     `clone TT as T1 with op c <- -5.` compiles; `print T1.ge0c` gives `axiom ge0c: 0 <= -5`;
     and `lemma boom : false. proof. smt(T1.ge0c). qed.` succeeds. Item 3's conclusion survives
     on other grounds, but not for the reason stated. **Forking is not the cheap escape**: it
   changes the TCB identically while creating a silently divergent copy of an upstream artifact. It
   forfeits *"we reused the published, independently-reviewed artifact; only the +C delta is ours"* — a
   large part of why this port is credible, and not recoverable by careful commit messages. Accordingly,
   `scratch/f1probe/base3/` and the other derived relaxed copies are **gitignored**; only generators and
   hand-written probes are tracked.
4. **Padding C10's encoding with 3 constant-zero digits to reach `len = 46`.** The padded scheme's pk is
   `th_multi` over 46 chain-ends vs the deployed 43 — a different pk, a different scheme; security does
   not transfer without a fresh reduction.
5. **Leaning on `constsum_encoding_is_two_encodings`** (`drafts/SphincsC10Content.ec:192`) as the repair.
   NEW NEGATIVE RESULT: its hypotheses are **global** over `msgWOTS` (`injective E` *and* constant
   `digitsum`) and are **jointly unsatisfiable** at deployed geometry (the `TARGET_SUM=205` layer holds
   2^114.09 points; the largest layer 2^123.76). **PART B is vacuous at deployed parameters.** It remains
   a correct result about the antichain half.

**IF a deployed-parameter claim is ever wanted — the exact shape.** MM45's `two_encodings` **bundles**
antichain image **and** global injectivity. WOTS+C supplies the first via the constant-sum gate and
**provably cannot** supply the second (an injective antichain encoding of 2^128 messages needs
`len >= 45` at `w = 8`; deployed 43 is two chains short *by counting alone*). Injectivity is therefore not
inconvenient, it is **impossible**. A sound development must **drop the injectivity half and charge
same-encoding pairs to a computational term** — the S-TCR-on-Th+C summand of ePrint 2022/778 Thm 5.2:
a **parametric WOTS+C layer** with free `(n, w, len)`, `len2 = 0` admissible, and a **gate-restricted**
antichain hypothesis proven on the `predC`-valid set, **derived from** MM45 rather than editing it so the
vendored artifact stays pristine. A separate project phase, not a patch.
