(* MUST PASS (positive control at a DIFFERENT pin).  Same corollary with the
   pin one ABOVE c.  Purpose: show the capstone consumes the INEQUALITY
   `c <= p_tgts` and nothing narrower -- i.e. 262656 was chosen because it is
   the LEAST admissible value, not because the capstone needs that exact
   integer.  Together with CtlCapstonePinOffByOne (which fails at 262655) this
   brackets the pin from both sides. *)
...
lemma ctl_c_le_hi : p_tgts = ctl_p_tgts_hi => c <= p_tgts.
proof. by move=> ->; rewrite c10_c_closed /c10_c /ctl_p_tgts_hi. qed.

lemma CTL_EUFCMA_SPHINCS_PLUS_C10_AT_PIN_PLUS_ONE
  ...
  &m :
    ...
    emb_in = c10_embg =>
    (* ---- CONTROL DELTA: the pin, one ABOVE the computed c. ---- *)
    p_tgts = ctl_p_tgts_hi =>
    0%r <= mkg_adv =>
