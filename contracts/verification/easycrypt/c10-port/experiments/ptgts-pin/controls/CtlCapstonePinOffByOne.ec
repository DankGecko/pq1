(* MUST FAIL.  The CAPSTONE-level off-by-one: PTgtsPinCapstone.ec verbatim with
   the pin moved DOWN BY ONE (262655).  This is the control that matters -- the
   local arithmetic controls only test PTgtsPin.ec, whereas this one tests that
   the capstone corollary genuinely CONSUMES the discharge rather than carrying
   a premise nothing checks.  Expected: failure on ctl_c_le (the arithmetic),
   before the capstone application is even reached. *)
...
op ctl_p_tgts_lo : int = 262655.        (* c10_p_tgts - 1 *)

lemma ctl_c_le : p_tgts = ctl_p_tgts_lo => c <= p_tgts.
proof. by move=> ->; rewrite c10_c_closed /c10_c /ctl_p_tgts_lo. qed.

lemma CTL_EUFCMA_SPHINCS_PLUS_C10_AT_OFFBYONE_PIN
  ...
  &m :
    ...
    emb_in = c10_embg =>
    (* ---- CONTROL DELTA: the pin, one BELOW the computed c. ---- *)
    p_tgts = ctl_p_tgts_lo =>
    0%r <= mkg_adv =>
