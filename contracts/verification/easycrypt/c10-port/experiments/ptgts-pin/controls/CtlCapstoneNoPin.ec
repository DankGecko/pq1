(* MUST FAIL (load-bearing control).  PTgtsPinCapstone.ec with the pin premise
   REMOVED ALTOGETHER.  An unused premise compiles in EasyCrypt, so "it
   compiles with the pin" is NOT by itself evidence that the pin does anything.
   This file is that evidence: with nothing supplying `c <= p_tgts`, the
   capstone application must not typecheck.  Expected: failure at the `exact`. *)
...
lemma CTL_EUFCMA_SPHINCS_PLUS_C10_WITH_NO_PIN
  ...
  &m :
    ...
    emb_in = c10_embg =>
    (* ---- CONTROL DELTA: the target-count premise is DELETED OUTRIGHT. ---- *)
    0%r <= mkg_adv =>
...
proof.
move=> hcard hemb hmkg hencb hdf8n hdflen hdf2 hdfnk htree hN2.
