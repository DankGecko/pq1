...
lemma EUFNAGCMA_FLSLXMSSMTTWCESNPRF_Unfolded
  ...
  <=   (w - 2)%r
     ...
     + Pr[FSSLXMTWES.TRHC_TCR.SM_DT_TCR_C(R_SMDTTCRCTRH_C(A_ht),
            FSSLXMTWES.TRHC_TCR.O_SMDTTCR_Default,
            FSSLXMTWES.TRHC.O_THFC_Default).main() @ &m : res]
     (* THREADED (experiment wots-badenc): MM45's admitted encoder injectivity is
        gone from the WOTS leg; what replaces it is this explicit encoding-
        collision term, at the SAME adversary instantiation as the UD/TCR/PRE
        terms above.  It is not a new assumption -- it is the admit, made
        visible and priced. *)
