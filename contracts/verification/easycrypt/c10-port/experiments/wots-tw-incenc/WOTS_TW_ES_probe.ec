...
(* ===================================================================== *)
(* PROBE-ONLY SCAFFOLD -- NOT A RESULT, NOT PART OF THE REPAIR.          *)
(* PROBE_enc_inj IS injectivity, i.e. exactly the property the whole      *)
(* experiment exists to REMOVE. Its presence makes THIS FILE vacuous at   *)
(* C10 geometry. Sole purpose: let the compile proceed PAST the single    *)
(* known gap so that any FOURTH site needing injectivity is revealed.     *)
(* The real repair discharges this by a T-COLL-RES game hop instead.      *)
(* ===================================================================== *)
axiom PROBE_enc_inj (m m' : msgWOTS) :
  m <> m' => encode_msgWOTS m <> encode_msgWOTS m'.

lemma nhchwcoll_hchwpre_msg (ps : pseed) (ad : adrs) (m m' : msgWOTS) (sig sig' : sigWOTS) :
     m <> m'
  => !has_chwcoll ps ad (encode_msgWOTS m) (encode_msgWOTS m') sig sig'
  => has_chwpre ps ad (encode_msgWOTS m) (encode_msgWOTS m') sig sig'.
proof. by move=> hne; apply nhchwcoll_hchwpre; apply PROBE_enc_inj. qed.
