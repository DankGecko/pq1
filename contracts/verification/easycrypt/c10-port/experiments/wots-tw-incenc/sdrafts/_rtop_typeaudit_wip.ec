...
(* ==========================================================================
   ...
   -- neither the original `A_wf_ht` (`all (<> dfC)`), nor its member-set
   ...
   independent of the adversary:
     * `c <= p_tgts`, the two `emb_tw` address facts and the encode-compat fact
       -- WOTS+C leg side-conditions inherited verbatim from :1683/:875;
     * the FOUR `dfC <> ...` member-separation FLAG facts -- parameter
...
lemma leaf_reduction_MEUFGCMAWOTSC_bound_Rtop
  ...
  <=   Pr[M_EUF_GCMA_WOTSTWESNPRF(R_int_WOTSTW(R_MEUFGCMAWOTSC_EUFNAGCMA_C(R_top(F))),
     ...
     + Pr[S_TCR_C_Int_MA(R_int_STCRC(R_MEUFGCMAWOTSC_EUFNAGCMA_C(R_top(F))),
                         STCRC_WC.O_STCRC_Default).main() @ &m : res].
proof.
move=> hc hembdisj hembinj hencb h1 h2 h3 h4.
apply (leaf_reduction_MEUFGCMAWOTSC_bound_members4 (R_top(F)) &m hc hembdisj hembinj hencb h1 h2 h3 h4).
...
(* A forger that uses the edge: one collection query at the CHALLENGED member
   `dfC` (same shape as `A_ht_dfC`, :819). *)
...
(* With the forbidden edge in place the audit postcondition is FALSE, given the
   four separation FLAG facts that put `dfC` outside the four-member set. *)
...
(* ==========================================================================
   ...
   NOT ACHIEVED -- carried forward, unchanged in character:
     ...
     * The four `dfC <> ...` separation facts remain THREADED HYPOTHESES (`dfC`
       is abstract here), as at (2) above.  `R_top` itself consumes only three
       of them ({8n, 8n*2, 8n*k}); `dfC <> 8*n*len` is consumed by `R_leaf`'s own
...
(* ==========================================================================
   ...
   are NOT unified):
     ...
     A_wf_ht     over  A_ht(O_THFC_MA)                      [member axis, <> dfC]
...
(* --------------------------------------------------------------------------
   ...
   theorem's premise (a) is the COLLAPSED form (`all (p.`1 <> dfC)`), and (C.6)
...
(* ==========================================================================
   ...
   ACHIEVED (all 0-admit, 0-axiom; `bash ec-certify.sh` on this file):
     ...
     * `R_top_A_wf_ht`     -- premise (a), member axis, in the EXACT capstone
       form (`all (p.`1 <> dfC)`), which (C.3)/(C.6) had not produced over
       `R_top(F, O_THFC_MA).choose`.  It carries the four `dfC <> _` FLAG facts,
   ...
   (R1) THE CAPSTONE IS NOT ASSEMBLED HERE.  `EUFNAGCMA_FLSLXMSSMTTWCESNPRF`
        ...
        and these lemmas are not yet in one compilation unit.  Until they are,
        "the premises are discharged" is a claim about four separate lemmas
        whose statements were copied from the theorem, not a proved implication.
   ...
   (R5) `hencb`, `c <= p_tgts`, the two `emb_tw` address facts and the four
        `dfC <> _` facts remain CARRIED on the component theorem.  They are not
