(* ==========================================================================
   ...
   (NEVER admits) because they are load-bearing and, for mtree_*, FALSE-AT-ZERO:
     ...
     * emb_tw disjointness + injectivity        -- **NO LONGER CARRIED** (2026-07-25).
       ...
       four dfC width facts) are conditions on ABSTRACT MODEL CONSTANTS (p_tgts is
       declared with only `0 <= p_tgts`; dfC = size (emb_in witness) over the
     ...
     * dfC <> {8n, 8n*len, 8n*2, 8n*k}          (C10 serialisation-width facts; the
   ...
   [carried PREMISES of this lemma -- not axioms; see (2)]
     * mtree_* (H-TREE-MULTI, false-at-zero), c<=p_tgts, encode-compat, 0<=mkg_adv,
       dfC facts (now incl. 8n*k).  (The 3 member hoare premises are NO LONGER
...
proof.
  ...
  (* ---- member axes at A_ht := R_top_C(F): ALL FOUR DISCHARGED via the PROVEN
     ports (RtopCSoundness, 2026-07-24 hop6b closure).  A_wf_ht (member/dfC axis) via
  ...
  have hHT := EUFNAGCMA_FLSLXMSSMTTWCESNPRF (R_top_C(F)) &m hc emb_disj_concrete hencb
...
(* (R4b) The two REAL-valued premises of the theorem above are JOINTLY SATISFIABLE
   ...
   satisfiable it certainly is.)  scratch/synth_repaired_canary3.ec discharges both
   OUTRIGHT while instantiating the theorem, so the capstone is genuinely
   APPLICABLE, not merely well-typed. *)
