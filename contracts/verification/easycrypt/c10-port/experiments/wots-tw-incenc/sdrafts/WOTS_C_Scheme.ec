...
module WOTS_C_ES = {
  ...
  proc sign(sk : skWOTS * pseed * adrs, m : msgWOTS) : sigWOTS * cntr = {
    var ps : pseed;
    var ad : adrs;
    var skWOTS : skWOTS;
  ...
  proc verify(pk : pkWOTSTW, m : msgWOTS, sigc : sigWOTS * cntr) : bool = {
    var ps : pseed;
    var ad : adrs;
...
lemma sign_counter_predC (skW : skWOTS) (ps0 : pseed) (ad0 : adrs) (m0 : msgWOTS) :
...
module type Oracle_MEUFGCMA_WOTSC = {
  ...
  proc get(i : int) : adrs * msgWOTS * pkWOTS * (sigWOTS * cntr)
...
module O_MEUFGCMA_WOTSC_Default : Oracle_MEUFGCMA_WOTSC = {
  var ps : pseed
  var qs : (adrs * msgWOTS * pkWOTS * (sigWOTS * cntr)) list
  ...
  proc get_addresses() : adrs list = {
    return map (fun (q : adrs * msgWOTS * pkWOTS * (sigWOTS * cntr)) => q.`1) qs;
  }
  ...
  proc dist_addresses() : bool = {
    return uniq_wgpidxs (map (fun (q : adrs * msgWOTS * pkWOTS * (sigWOTS * cntr)) => q.`1) qs);
  }
}.
