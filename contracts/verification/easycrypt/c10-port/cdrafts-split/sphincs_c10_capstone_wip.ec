(* ==========================================================================
   sphincs_c10_capstone_wip.ec  --  THE SPHINCS+C10 EUF-CMA CAPSTONE (base A).
   ...
   !! WAVE-9/10 CAVEAT (2026-07-21) -- READ BEFORE TREATING THE FORS LEG AS "PROVEN":
   The FORS+C10 bound hF (M.EUFCMA_MFORSC10) is ITSELF a GENUINE MEANINGFUL conditional
   (Wave-10 accept-all probe + negative control: NO hidden fverify-hypothesis; content =
   ITSRC10 + the load-bearing mtree premise).  But the hop5 SEAM connecting p_vt to it
   is BLOCKED against the ABSTRACT M: fverify:=false zeroes Pr[RHS] (machine-checked
   rhs_zero_fverify_false) and M.mkeygen cannot couple to V_C's ps-independent cube (D2).
   So the FORS leg is CONDITIONAL on a CONCRETE PROCEDURAL M-FORS+C game + hop5 over it
   (fix pinned at `hF`); it is not yet a connected proven leg.  The hypertree leg (hHT)
   is UNAFFECTED (concrete NAGCMA win; hop-6 is a canary-verified 0-admit byequiv).
   ...
   ------ (0) LHS NATURE  (abstract, flagged; concrete-scheme is the residual) --
   There is NO concrete `module SPHINCS_PLUS_C10 : Scheme` and NO concrete
   `EUF_CMA(SPHINCS_PLUS_C10, F, O_CMA_Default)` game in this port.  The +C
   scheme appears only as SIMULATED oracles inside reductions (R_top.O_CMA
   XmssmtCC_All.ec:9457).  The top +C forger TYPE exists (`Adv_EUFCMA_C`,
   XmssmtCC_All.ec:9428) and we quantify over it, but the SPHINCS+C10 forgery
   probability itself is the ABSTRACT real `p_sphincs_c`.  Building the concrete
   scheme = MM45 `module SPHINCS_PLUS : Scheme` (SPHINCS_PLUS.ec:957) + the two
   Thm-5.2 substitutions ((i) FORS branch -> FORS+C10 with the conditioned draw
   `mk <$ dcond dmkey (good_fors m)`, (ii) hypertree layer already +C) and then
   instantiating the generic `EUF_CMA(.,.,.)` game.  MM45's FX proof is
   section-local and inlines concrete reductions over `mco`, so there is NO
   clone/instantiate shortcut -- this is the multi-month scheme-module remainder
   and is the STATED CONCRETE-LHS RESIDUAL.
...
lemma EUFCMA_SPHINCS_PLUS_C10
  ...
  (* Abstract reals: the SPHINCS+C10 forgery prob (LHS) + the two PRF advantages. *)
  (p_sphincs_c skg_adv mkg_adv : real)
...
proof.
  ...
  (* !! WAVE-9/10 CAVEAT (2026-07-21, machine-checked drafts/rtop_c_vt_wip.ec +
     Wave-10 discriminating probes).  hF ITSELF is a GENUINE MEANINGFUL bound: the
     fverify:=accept-all probe + a negative control prove EUFCMA_MFORSC10 hides NO
     fverify-soundness hypothesis -- its content is exactly (ITSRC10 assumption) + the
     LOAD-BEARING H-TREE-MULTI mtree premise, which scales the RHS.  The vacuity is NOT
     in hF.  The GAP is the hop5 SEAM: M = FORS_C10_Multi.MFORSC10 has UNCONSTRAINED
     abstract fverify/mkeygen/fsign, so (a) fverify:=false zeroes Pr[RHS]
     (rhs_zero_fverify_false, 0-admit) and (b) M.mkeygen derives the FORS cube from ps
     while V_C samples it ps-INDEPENDENTLY (D2) -- so LeqPr_VT_C (hop5) cannot couple
     p_vt to this ABSTRACT game.  FIX (structural, pinned): a CONCRETE PROCEDURAL
     M-FORS+C game -- procedural sampling keygen a la M_FORS_ES_NPRF.keygen (matches
     V_C's draw by construction), concrete routed sign, verify by reconstructed-pkFORS
     eq + predC_fors -- re-establish EUFCMA_MFORSC10 over it, then port MM45 VT coupling
     (SPHINCS_PLUS.ec:3143-3174/:3307-3327).  Until hop5 lands over the concrete game,
     the FORS leg is CONDITIONAL on that fix.  (VF/hypertree leg hHT is UNAFFECTED --
     concrete NAGCMA win; hop-6 LeqPr_VF_C is a canary-verified 0-admit byequiv.) *)
  ...
  (* hop1  [genuinely-open]  Orig -> PRFPRF (materialization).
     ...
     (prf_hop_wip.ec:614-625).  MISSING: the deterministic-recomputation
     equivalence for the +C on-demand -> materialized cube. *)
  have hop1 : p_sphincs_c <= p_prfprf.
  + admit.
  ...
  (* hop3  NPRFPRF <= NPRFNPRF + mkg_adv.
     ...
     C10 models the message key as a fresh, non-memoized, +C-conditioned draw
     `mk <$ dcond dmkey (good_fors m)` on EVERY game of the chain (the message key
     is already the OUTPUT of the message-key idealisation composed with the +C
     conditioning; `mkg` is never applied in-chain, grep-verified).  So a faithful
     NPRFNPRF is DEFINITIONALLY NPRFPRF: `p_nprfprf = p_nprfnprf` by sim, and the
     MM45 paid |MKG-PRF| triangle (SPHINCS_PLUS.ec:3055) does NOT port.  Hence
     `mkg_adv` here is a PHANTOM in-chain summand -- a SOUND but silently-zeroable
     over-estimate, kept only so the bound needs no re-derivation.  The GENUINE MKG
     idealisation is real but lives at a SEPARATE pre-hop-1 boundary (the deployed
     scheme keys the grind on sk_seed via the fors.rs nonce loop; idealising that
     keyed salted-grinder to this dcond model is an RO step needing a primitive
     with salt+nonce in_t and multi-query-per-signature, which MM45's message-only
     MKG_PRF clone cannot express).  So: this capstone bounds the IDEALISED-mk
     model; mkg_adv correctly reads as that pre-hop-1 boundary term (a documented
     open refinement to reach the deployed scheme), NOT as a discharged in-chain
     hop.  Tightening option (not applied, to avoid re-deriving the bound): make
     hop3 the identity `p_nprfprf <= p_nprfnprf` and drop mkg_adv from the sum. *)
