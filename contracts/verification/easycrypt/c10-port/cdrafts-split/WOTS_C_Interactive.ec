...
(* ==========================================================================
   ...
   the batch proof (`STCR_C.ec` `Col`): it is a LOCAL clone with
       in_t <- dgstblock * cntr,  diff_t <- unit,  fc <- (fun _ pp tw x => ThC ..)
...
(* ==========================================================================
   ...
   collection `STCRC_WC.Col.Oracle_THFC` (STCR_C.ec:96, in_t = dgstblock*cntr).
...
(* ==========================================================================
   ...
   Since (WOTS_C_Real.ec)
        ThC ps tw m c = thfc (size (emb_in (m, c))) ps (emb_tw tw) (emb_in (m, c))
   and `emb_in : dgstblock * cntr -> dgst` is an ABSTRACT op with NO constraints, the
   ...
   only if BOTH of the following hold on the concrete serialisation:
     ...
     (LEN)  size (emb_in (m,j)) = size (emb_in (m',ctr))
            — so both ThC evaluations select the SAME collection member
              `thfc dfC0` at a fixed length index dfC0 (FC membership condition:
   ...
   them.  The former `embinj` premise is GONE — see the VACUITY REPAIR block.):
     ...
     emb_in_len : forall (x y : dgstblock * cntr), size (emb_in x) = size (emb_in y)
     emb_in_inj : forall (x y : dgstblock * cntr), emb_in x = emb_in y => x = y
   ...
   every `x` (satisfies `emb_in_len`, with dfC0 = L).  Fixed-width field
   ...
   `dgstblock * cntr` is FINITE (`cntr` is finite — carried as
   `STCRC_WC.G.CntrFT.enum_spec`; `dgstblock` is a fixed-length word list), so a
...
(* The single fixed length index every `emb_in` output has under (LEN).  A concrete
   `op` DEFINITION (not an axiom): under `emb_in_len`, `size (emb_in x) = dfC0` for
   every `x` (instantiate y := witness).  This is the `df` of the FC membership
   `exists df, fc df = f` for the member Th+C challenges. *)
(* ROUTE (D): TWO member indices, one per projection.  Both are concrete `op`
   DEFINITIONS (not axioms), and they are DISTINCT under (LEN). *)
op dfC0 : int = size (emb_in0 witness).
op dfC1 : int = size (emb_in1 witness).

(* `thfc dfC0` is ONE fixed member of the size-indexed collection `thfc` (`fc = thfc`
   in FC), witnessed at index dfC0 — the `in_collection`-shape membership
   (TweakableHashFunctions.eca:565) of the function a standard single-member
   SM-DT-TCR(+C) game would challenge. *)
lemma ThC_member : exists (df : int), thfc df = thfc dfC0.
proof. by exists dfC0. qed.

lemma ThC_member1 : exists (df : int), thfc df = thfc dfC1.
proof. by exists dfC1. qed.

(* The two message-compression members are distinct members of `thfc`. *)
(* STRENGTHENED 2026-08-01: the constant-width premise was REDUNDANT.  dfC0 and
   dfC1 are both taken at the SAME `witness` -- 1 + size (emb_in witness) and
   2 + size (emb_in witness) -- so they differ for EVERY emb_in, with no
   hypothesis at all.  Found by the refutation of a (false) review finding that
   claimed this separation was silently conditional. *)
lemma ThC_members_distinct : dfC0 <> dfC1.
proof. by rewrite /dfC0 /dfC1 emb_in0_size emb_in1_size; smt(). qed.

(* CORE FACT 1 (LEN):  every `ThC` evaluation is the SAME collection member
   `thfc dfC0` applied at tweak `emb_tw tw` on input `emb_in (m,c)`.  Hence in the
   collision `ThC pp tw m j = ThC pp tw m' ctr` BOTH sides hit the single member
   `thfc dfC0` at the single tweak `emb_tw tw` — a single-member application, not a
   cross-member coincidence. *)
lemma ThC_same_member (ps : pseed) (tw : adrs) (m : dgstblock) (c : cntr) :
  (forall (x y : dgstblock * cntr), size (emb_in x) = size (emb_in y)) =>
  ThC ps tw m c = join_dgst (thfc dfC0 ps (emb_tw tw) (emb_in0 (m, c)))
                            (thfc dfC1 ps (emb_tw tw) (emb_in1 (m, c))).
proof.
  move=> emb_in_len; rewrite /ThC /dfC0 /dfC1 !emb_in0_size !emb_in1_size.
  by rewrite (emb_in_len (m, c) witness).
qed.

(* PROJECTION-COLLISION REDUCTION (route (D)'s S-TCR repair).  `join_dgst` is
   injective, so a collision in the WIDE digest is a collision in EACH half --
   in particular one can always be read off at the single fixed member `dfC0`.
   This is what replaces the old "ThC IS one member by definition" argument,
   which route (D) no longer has available. *)
lemma ThC_coll_projects (ps : pseed) (tw : adrs) (m m' : dgstblock) (j c : cntr) :
  (forall (x y : dgstblock * cntr), size (emb_in x) = size (emb_in y)) =>
  ThC ps tw m j = ThC ps tw m' c =>
  thfc dfC0 ps (emb_tw tw) (emb_in0 (m, j))
    = thfc dfC0 ps (emb_tw tw) (emb_in0 (m', c)).
proof.
  move=> hlen; rewrite (ThC_same_member _ _ _ _ hlen) (ThC_same_member _ _ _ _ hlen).
  by move=> /join_dgst_inj [] h _.
qed.
...
lemma ThC_genuine_2ndpreimage (m m' : dgstblock) (j ctr : cntr) :
...
(* FAITHFULNESS LEMMA (the caveat discharge).  Fed the two conjuncts of
   ...
   it yields a genuine STANDARD single-member SM-DT-TCR(+C) second-preimage:
     * SAME member       : `thfc dfC0`      (CORE FACT 1 / LEN);
     * SAME tweak        : `emb_tw tw`      (syntactically shared — no proof needed);
     * SAME output       : the collision equation, transported to `thfc dfC0`;
   ...
   member `f := thfc dfC0` at tweak `emb_tw tw` — so the `S_TCR_C_Int` term is a
   single-member TCR break, discharging the audit's caveat (not multi-member, not
   trivial). *)
lemma S_TCR_C_Int_win_2ndpreimage
  (pp : pseed) (tw : adrs) (m m' : dgstblock) (j ctr : cntr) :
  (forall (x y : dgstblock * cntr), size (emb_in x) = size (emb_in y)) =>
  (forall (x y : dgstblock * cntr), emb_in x = emb_in y => x = y) =>
  m' <> m =>
  ThC pp tw m j = ThC pp tw m' ctr =>
     emb_in0 (m, j) <> emb_in0 (m', ctr)
  /\ thfc dfC0 pp (emb_tw tw) (emb_in0 (m, j))
   = thfc dfC0 pp (emb_tw tw) (emb_in0 (m', ctr)).
proof.
  move=> emb_in_len emb_in_inj hne hcoll; split.
  + apply/negP => heq; have := emb_in0_inj (m, j) (m', ctr) emb_in_inj heq.
    by smt().
  exact (ThC_coll_projects pp tw m m' j ctr emb_in_len hcoll).
qed.
...
(* PREDICATE BRIDGE (the full standard-game statement, at the win-predicate level).
   ...
   for the STANDARD single fixed member `f := thfc dfC0` (ThC_member), under
   ...
   Pr[SM_DT_TCR_C(R'(A))]` over a fresh `thfc dfC0`-membered `SMDTTCRC` clone is
...
lemma S_TCR_C_Int_win_implies_SMDTTCRC
  ...
  (* SM_DT_TCR_C win (member thfc dfC0, tweak emb_tw tw, inputs emb_in(m,j)/emb_in(m',ctr)): *)
     0 <= i < nrts
  /\ 0 <= nrts <= t
  /\ uniq (map emb_tw twsOraw)
...
module (R_int_STCRC (A : Adv_MEUFGCMA_WOTSC) : Adv_ISTCRC)
  ...
  module O_wrap : Oracle_MEUFGCMA_WOTSC = {
    ...
    proc query(wad : wadrs, m : dgstblock) : pkWOTS * (sigWOTS * cntr) = {
      var ad : adrs;
      var d : msgWOTS;      (* ThC's OUTPUT, from the S-TCR(+C) oracle *)
      var c : cntr;
      var em : emsgWOTS;
...
(* ==========================================================================
   ...
   VALID WOTS address — and the reason is INTERFACE TYPING, not a runtime check:
     ...
     * the ONLY path from A into `ts` is `R_int_STCRC`'s own `O_wrap.query`, whose
       signature is `query(wad : wadrs, m : dgstblock)` and which registers exactly
       `WAddress.val wad`;
...
lemma R_ts_allvalid_FC
  ...
  hoare[ R_int_STCRC(A, STCRC_WC.O_STCRC_Default, FC.O_THFC_Default).pick :
           ...
           all (fun (t : adrs * dgstblock * cntr) => valid_wadrs t.`1)
               STCRC_WC.O_STCRC_Default.ts ].
proof.
proc.
call (: all (fun (t : adrs * dgstblock * cntr) => valid_wadrs t.`1)
            STCRC_WC.O_STCRC_Default.ts).
+ (* O_wrap.query: registers exactly WAddress.val wad *)
  proc; inline*.
  wp.
  while (all (fun (t : adrs * dgstblock * cntr) => valid_wadrs t.`1)
             STCRC_WC.O_STCRC_Default.ts).
  - wp.
    while (all (fun (t : adrs * dgstblock * cntr) => valid_wadrs t.`1)
               STCRC_WC.O_STCRC_Default.ts).
...
lemma allvalid_map_fst (ts : (adrs * dgstblock * cntr) list) :
     all (fun (t : adrs * dgstblock * cntr) => valid_wadrs t.`1) ts
  => all valid_wadrs (map (fun (tmc : adrs * dgstblock * cntr) => tmc.`1) ts).
...
op projq (q : adrs * dgstblock * pkWOTS * (sigWOTS * cntr)) : adrs * dgstblock * cntr =
  (q.`1, q.`2, q.`4.`2).
...
proof.
  ...
  have hfst : map (fun (tmc : adrs * dgstblock * cntr) => tmc.`1)
                  STCRC_WC.O_STCRC_Default.ts{2}
  ...
  apply (interactive_success_transfer pp{2} rR.`1
           ...
           (map (fun (tmc : adrs * dgstblock * cntr) => tmc.`1) STCRC_WC.O_STCRC_Default.ts{2})
           FC.O_THFC_Default.tws{2}
...
(* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- *)
(* ROUTE (D): back to the ORIGINAL one-oracle interface.  The wide digest is
   assembled from TWO queries to the SAME collection oracle `OC`, joined
   outside it -- so no second collection oracle is needed and MM45's own
   theorem applies unchanged.  Cost, as flagged by both review legs: the grind
   loop now makes 2 collection queries per candidate counter. *)
module (R_int_WOTSTW (A : Adv_MEUFGCMA_WOTSC) : Adv_MEUFGCMA_WOTSTWESNPRF)
  ...
  module O_wrap : Oracle_MEUFGCMA_WOTSC = {
    ...
    proc query(wad : wadrs, m : dgstblock) : pkWOTS * (sigWOTS * cntr) = {
      var ad : adrs;
      var d, y : msgWOTS;   (* ThC outputs *)
      var y0, y1, d0, d1 : dgstblock;   (* the two projections *)
      var c, seed : cntr;
      var cs : cntr list;
      var found : bool;
      var pk : pkWOTS;
      ...
      while (!found /\ cs <> []) {
        ...
        y0 <@ OC.query(emb_tw ad, emb_in0 (m, seed));
        y1 <@ OC.query(emb_tw ad, emb_in1 (m, seed));
        y  <- join_dgst y0 y1;
        if (predC y) { c <- seed; found <- true; }
        cs <- behead cs;
      }
      d0 <@ OC.query(emb_tw ad, emb_in0 (m, c));
      d1 <@ OC.query(emb_tw ad, emb_in1 (m, c));
      d  <- join_dgst d0 d1;
  ...
  proc forge(ps : pseed) : int * msgWOTS * sigWOTS = {
    var i : int;
    var m' : dgstblock;      (* A's +C forgery message is a NODE *)
    var sigc' : sigWOTS * cntr;
...
lemma ThC_E (ps : pseed) (ad : adrs) (m : dgstblock) (cc : cntr) :
  join_dgst (thfc (size (emb_in0 (m, cc))) ps (emb_tw ad) (emb_in0 (m, cc)))
            (thfc (size (emb_in1 (m, cc))) ps (emb_tw ad) (emb_in1 (m, cc)))
  = ThC ps ad m cc.
proof. by rewrite /ThC. qed.

(* --------------------------------------------------------------------------
   THE GATE, UNCONDITIONALLY -- with the grind-failure escape hatch.

   This is the hinge of the 2026-07-31 N2 removal from the COUPLING.  Read
   `grindP_iff` (Grind.ec:104) at the STCRC_WC clone (`f <- ThC`,
   `prop <- predC`, STCR_C.ec:65-72):

     predC (ThC ps ad m (grind ps ad m))  <=>  ! grind_fails ps ad m

   so the disjunction below is a TAUTOLOGY -- a case split on a boolean, needing
   NO premise whatsoever.  Previously `query_eq_tw`/`choose_tw` carried the bare
   gate `predC (ThC .. stored_ctr)` as a forward invariant, which is NOT free and
   is exactly what forced N2 into the coupling.  Weakening the invariant by the
   `grind_fails` disjunct makes it free, and the disjunct is discharged LATER --
   either by N2 (`interactive_hop2`, statement unchanged) or by `!gfail`
   (`GFailCharged.interactive_hop2_charged`, where it becomes a charged term).

   Note the direction: the new invariant is strictly WEAKER than the old, so
   every consumer that could use the old can still be served -- the obligation
   just moves from the oracle-coupling to the single point of consumption.
   -------------------------------------------------------------------------- *)
lemma grind_disj (ps : pseed) (ad : adrs) (m : dgstblock) :
  STCRC_WC.G.grind_fails ps ad m \/ predC (ThC ps ad m (grindC ps ad m)).
proof.
case: (STCRC_WC.G.grind_fails ps ad m) => hf; first by left.
by right; rewrite grindC_E; move: hf; rewrite -STCRC_WC.G.grindP_iff.
qed.
...
lemma query_eq_tw
  ...
  (forall (p : pseed) (a : adrs) (x : dgstblock) (cc : cntr),
     encode_msgWOTS_C p a x cc = encode_msgWOTS (ThC p a x cc)) =>
  (* N2 USED TO SIT HERE and no longer does (2026-07-31).  The gate invariant
     below now carries the `grind_fails` disjunct, which `grind_disj` supplies
     unconditionally -- so this coupling is N2-FREE.  Removing the premise is the
     whole point: it is what lets `interactive_hop2_charged` reuse this same
     lemma with the disjunct discharged by `!gfail` instead of by N2. *)
  equiv[ O_MEUFGCMA_WOTSC_Default.query
...
seq 0 1 : (
  ...
  /\ all (fun (q : adrs * dgstblock * pkWOTS * (sigWOTS * cntr)) =>
            STCRC_WC.G.grind_fails O_MEUFGCMA_WOTSC_Default.ps{1} q.`1 q.`2 \/ predC (ThC O_MEUFGCMA_WOTSC_Default.ps{1} q.`1 q.`2 q.`4.`2))
         O_MEUFGCMA_WOTSC_Default.qs{1}
  (* the two oracles share the seed: the grind runs on the WIDE one *)).
+ while{2} (
       ad{2} = WAddress.val wad{2}
       (* the two oracles share the public seed (OC.init(ps); OC_M.init(ps)) --
          the grind now runs on the WIDE oracle while `grindC` is stated at the
          narrow one, so the coupling needs this explicitly *)
    ...
    /\ all (fun (q : adrs * dgstblock * pkWOTS * (sigWOTS * cntr)) =>
           ...
           O_MEUFGCMA_WOTSC_Default.qs{1})
      (size cs{2}).
  (* ROUTE (D): the grind loop queries the ONE collection oracle TWICE per
     candidate (the two projections), so the narrow transcript grows by TWO
     `emb_tw` entries per iteration.  The `rcons`/`emb_tw` reasoning is
     therefore BACK -- and doubled.  [CORRECTED 2026-08-01: the previous
     comment here claimed this reasoning was "gone", which was true only of the
     abandoned second-family design.] *)
  + move=> &1 z; wp; skip => /> &hr *; rewrite !ThC_E /=.
    have htws : all (fun (t : adrs) => (t \in FC.O_THFC_Default.tws{1}) \/ exists (b : adrs), t = emb_tw b)
                    (rcons (rcons FC.O_THFC_Default.tws{hr} (emb_tw (WAddress.val wad{hr})))
                           (emb_tw (WAddress.val wad{hr})))
...
rnd.
skip => &1 &2 [tws_R Hpre].
(* ROUTE (D): the d-fetch is TWO queries, so the precondition carries two
   assignment groups; the transcript conjunct is a SINGLE double-`rcons`
   (`tws{2} = rcons (rcons tws_R tw1) tw2`), and `hd` equates d to the JOIN.
   Names read off the actual goal state (`easycrypt cli`), not guessed. *)
move: Hpre => [# htw1 hx1 hdf1 hy4 hd0 htw2 hx2 hdf2 hy5 htwseq hd1 hd hwad0 hm0 hpseq had0eq hwadm hmm hinv1 hinv2 hinv3 hqs2 hwadsr hadeq hceq hps1eq had1eq htwsall hgateall].
...
split.
(* ROUTE (D) [CORRECTED 2026-08-01]: the two projection queries go to the SAME
   narrow oracle, so side 2's transcript DOES grow -- by two `emb_tw ad`
   entries.  The previous comment here ("transcript unchanged") described the
   abandoned second-family design and was false for route (D). *)
+ have htws2 : all (fun (t : adrs) => (t \in FC.O_THFC_Default.tws{1})
                                      \/ exists (b : adrs), t = emb_tw b)
                   (rcons (rcons tws_R (emb_tw ad{2})) (emb_tw ad{2}))
    by rewrite -!cats1 !all_cat /=; smt().
  by rewrite htwseq htw1 htw2; exact htws2.
(* GATE INVARIANT for the freshly appended query.  The oracle stores grindC's
   counter, so `grind_disj` gives the (weakened) gate for the ground digest with
   NO premise; the older entries come straight from the precondition. *)
rewrite -cats1 all_cat /=.
split; first exact hgateall.
have hg := grind_disj O_MEUFGCMA_WOTSC_Default.ps{1} (WAddress.val wad{1}) m{1}.
smt(grindC_E).
qed.
...
lemma choose_tw
  ...
  (forall (p : pseed) (a : adrs) (x : dgstblock) (cc : cntr),
     encode_msgWOTS_C p a x cc = encode_msgWOTS (ThC p a x cc)) =>
  (* N2-FREE since 2026-07-31 -- see the note on `query_eq_tw`. *)
  equiv[ A(O_MEUFGCMA_WOTSC_Default, FC.O_THFC_Default).choose
...
proof.
  ...
  proc (   O_MEUFGCMA_WOTSC_Default.ps{1} = O_MEUFGCMA_WOTSTWESNPRF.ps{2}
        ...
        /\ all (fun (t : adrs) => t \in FC.O_THFC_Default.tws{1} \/ exists b, t = emb_tw b)
               FC.O_THFC_Default.tws{2}
        (* THE GATE CONJUNCT BELONGS **IN THE INVARIANT**, not only in the
           lemma's pre/post.  `proc INV` discharges the caller with
           `pre => INV` and `INV => post`; with the conjunct missing from INV
           the second implication is unprovable, `//` leaves it as goal #1, and
           the next `+ conseq (query_eq_tw ...)` then lands on a FIRST-ORDER
           IMPLICATION rather than the query-oracle equiv -- which is what
           produced the misleading `do not know how to combine equivF and _ and
           _`.  That error was a symptom of this omission, NOT of conseq
           plumbing; diagnosing it by reading the goal state rather than by
           inspecting the conseq line is what found it. *)
        /\ all (fun (q : adrs * dgstblock * pkWOTS * (sigWOTS * cntr)) =>
                  STCRC_WC.G.grind_fails O_MEUFGCMA_WOTSC_Default.ps{1} q.`1 q.`2 \/ predC (ThC O_MEUFGCMA_WOTSC_Default.ps{1} q.`1 q.`2 q.`4.`2))
               O_MEUFGCMA_WOTSC_Default.qs{1}
        (* the wide oracle's seed agreement must be IN THE INVARIANT too, for
           exactly the reason the comment above records for the gate conjunct:
           `proc INV` discharges the caller with pre => INV and INV => post, and
           query_eq_tw now carries this conjunct. *)
        /\ O_MEUFGCMA_WOTSC_Default.ps{1} = FC.O_THFC_Default.pp{2}) => //.
...
lemma interactive_hop2
  (A <: Adv_MEUFGCMA_WOTSC{-R_int_WOTSTW, -O_MEUFGCMA_WOTSC_Default,
    ...
    (forall (p : pseed) (a : adrs) (x : dgstblock) (cc : cntr),
       encode_msgWOTS_C p a x cc = encode_msgWOTS (ThC p a x cc)) =>
    (* N2: needed so the QUERIED digest satisfies the +C gate under the gated
       WOTS-TW game.  The forged digest's gate comes free from verifyC_TW. *)
...
proof.
  ...
  + exists* ps{1}, (nth witness O_MEUFGCMA_WOTSC_Default.qs{1} i{1}).`1, m'{1}, (sigc'{1}.`2);
    ...
    (* QUERIED-side gate conjunct: instantiate the maintained `all` at the very
       ...
       2026-07-31: the maintained `all` is now the WEAKER disjunctive form
    ...
    have hPqueried : predC (ThC O_MEUFGCMA_WOTSC_Default.ps{1}
                             ...
                             (nth witness O_MEUFGCMA_WOTSC_Default.qs{1} i{1}).`4.`2).
    + case: hd => [hfail|//].
      have hex := hN2 O_MEUFGCMA_WOTSC_Default.ps{1}
                    (nth witness O_MEUFGCMA_WOTSC_Default.qs{1} i{1}).`1
                    (nth witness O_MEUFGCMA_WOTSC_Default.qs{1} i{1}).`2.
      by move: hfail hex; rewrite STCRC_WC.G.grind_fails_iff; smt().
...
lemma interactive_D1
  (A <: Adv_MEUFGCMA_WOTSC{-R_int_STCRC, -R_int_WOTSTW, -O_MEUFGCMA_WOTSC_Default,
    ...
    (* N2 = "the +C grind always finds a good counter" -- App-D gap #1, the gap
       ...
       CORRECTION (this was stated WRONG when first committed): N2 is NOT
       discharged by `P_inhabited` / `targetSumReachable`.  Those give ONE
  ...
  <=   Pr[M_EUF_GCMA_WOTSTWESNPRF(R_int_WOTSTW(A),
     ...
     + Pr[S_TCR_C_Int(R_int_STCRC(A),
                      STCRC_WC.O_STCRC_Default, FC.O_THFC_Default).main() @ &m : res].
proof.
  move=> le_c_ptgts embdisj encb hN2 A_wf.
  have h1 := interactive_hop1 A &m le_c_ptgts embdisj encb A_wf.
  have h2 := interactive_hop2 A &m embdisj encb hN2.
  smt().
qed.
...
(* ==========================================================================
   ...
   the ThC target at its own member `dfC0 = size (emb_in _)`.  Then even though the
   pkco query and the target share the tweak `emb_tw ad`, they sit at DIFFERENT
   members — pkco at `8*n*len` (FL_SL_XMSS_MT_ES.ec:391), ThC at `dfC0` — so the
   ...
   single-member (`thfc dfC0`) TCR break needs is only that the CHALLENGED member
   `dfC0` is not opened at the target — which is precisely the member-aware
...
(* The ThC collection member.  `dfC0` (PART 1b, l.402) is `size (emb_in witness)`;
   under `emb_in_len` every ThC input has this length, so `thfc dfC0` is the single
...
(* The MAKE-OR-BREAK member-aware disjointness core:  if NO collection entry is
   recorded at the challenged member `dfC0`, the `dfC0`-tagged target list is
   `FC.disj_lists` from the collection — EVEN when a collection tweak coincides
   with a target tweak `emb_tw tw` (that entry sits at a member <> dfC0, so the
   PAIR (member, emb_tw tw) <> the target PAIR (dfC0, emb_tw tw)).  This is exactly
...
(* ROUTE (D): the CHALLENGED member is `dfC0` (the projection the
   collision is read off at -- see ThC_coll_projects).  The reduction's own
   `dfC1` traffic is at a DIFFERENT member, so as (member, tweak) PAIRS it is
   automatically separated from the dfC0-tagged targets; route (D) makes this
   side of the bookkeeping easier, not harder. *)
  (forall (p : int * adrs), p \in tws_ma => p.`1 <> dfC0) =>
  FC.disj_lists (map (fun (tw : adrs) => (dfC0, emb_tw tw)) twsOraw) tws_ma.
proof.
  move=> hmem; apply/hasPn => x /mapP [tw [_ ->]] /=.
  by apply/negP => hin; have := hmem (dfC0, emb_tw tw) hin.
qed.
...
(* --------------------------------------------------------------------------
   ...
   `dfC0 = size (emb_in _)` is the constant length of the C10 message-compression
   serialisation `M ‖ counter`: an 8*n-bit WOTS message digest `M` followed by the
   r-bit grinding counter (C10: r = 32), a fixed-width `8*n + r` bits.  Hence dfC0:
   ...
   Jointly satisfiable with `emb_in_len`/`emb_in_inj` (dfC0 = 8*n + r, r = 32,
...
(* Bridge: the three FLAG facts + "every recorded member is one of the three real
   ...
   the `member_sep_disj` premise `all members <> dfC0`. *)
...
(* The member-aware interactive S-TCR(+C) game.  IDENTICAL to `S_TCR_C_Int`
   (PART 1) except:
     ...
     * the disjointness win-conjunct is MEMBER-AWARE:
         FC.disj_lists (map (fun t => (dfC0, emb_tw t)) twsOraw) O_THFC_MA.tws_ma
       — the target lives at its FC coordinate `(dfC0, emb_tw tw)` (member dfC0,
   ...
   the SAME single-member `thfc dfC0` second-preimage assumption — see the
...
module S_TCR_C_Int_MA(A : Adv_ISTCRC, O : STCRC_WC.Oracle_STCRC) = {
  ...
  proc main() : bool = {
    ...
    return    0 <= i < nrts
           ...
           /\ m' <> m
           /\ ThC pp tw m j = ThC pp tw m' ctr
           /\ FC.disj_lists (map (fun (t : adrs) => (dfC0, emb_tw t)) twsOraw) twsMA;
  }
}.
...
(* --------------------------------------------------------------------------
   ...
   member `f := thfc dfC0` at tweak `emb_tw tw`, with the FAITHFUL member-aware
...
lemma S_TCR_C_Int_MA_win_implies_2ndpreimage
  ...
  (    0 <= i < nrts
    ...
    /\ m' <> m
    /\ ThC pp tw m j = ThC pp tw m' ctr
    /\ FC.disj_lists (map (fun (t0 : adrs) => (dfC0, emb_tw t0)) twsOraw) twsMA) =>
     0 <= i < nrts
  /\ 0 <= nrts <= t
  /\ uniq (map emb_tw twsOraw)
  /\ emb_in (m, j) <> emb_in (m', ctr)
...
(* The MEMBER-AWARE instance of the all-valid TARGET invariant (2026-07-25
   ...
   avoid; `member_sep_disj` needs only `p.`1 <> dfC0`). *)
lemma R_ts_allvalid
  (A <: Adv_MEUFGCMA_WOTSC{-R_int_STCRC, -O_MEUFGCMA_WOTSC_Default,
...
(* MEMBER-AWARE SUCCESS TRANSFER (tail).  The interactive analogue of
   ...
   pkco / trh tweaks all sit at members <> dfC0).  Only the PROVEN all-valid
   TARGET invariant (for `uniq (map emb_tw twsOraw)` via `emb_dist_valid`) and the
   member-separation enabler `all members <> dfC0` are needed — NO embdisj, NO
...
lemma interactive_success_transfer_MA
  ...
  (forall (p : int * adrs), p \in twsMA => p.`1 <> dfC0) =>
  uniq_wgpidxs twsOraw =>
  0 <= i < nrts => 0 <= nrts <= p_tgts =>
  m' <> m =>
  ThC pp tw m j = ThC pp tw m' ctr =>
...
(* ==========================================================================
   ...
   `A_wf_MA` — "the challenged member dfC0 is never opened by the collection
   oracle" — which the XMSS-MT hypertree adversary CAN satisfy (its pkco/trh/f
   collection queries all sit at members <> dfC0: FLAG facts dfC0<>{8n,8n*len,8n*2}).
   ========================================================================== *)
...
proof.
  ...
  have pickwf0 :
    equiv[ A(O_MEUFGCMA_WOTSC_Default, FC.O_THFC_Default).choose
         ~ R_int_STCRC(A, STCRC_WC.O_STCRC_Default, O_THFC_MA).pick :
             ...
             /\ STCRC_WC.O_STCRC_Default.ts{2}
                  = map projq O_MEUFGCMA_WOTSC_Default.qs{1}
             /\ all (fun (p : int * adrs) => p.`1 <> dfC0) O_THFC_MA.tws_ma{2} ].
  ...
  have hts : STCRC_WC.O_STCRC_Default.ts{2}
           = map projq O_MEUFGCMA_WOTSC_Default.qs{1} by smt().
  have hmem : forall (p : int * adrs), p \in O_THFC_MA.tws_ma{2} => p.`1 <> dfC0.
  + by move: Hpre; smt(allP).
  (* 2026-07-25: the all-valid TARGET invariant, carried through the `seq` above. *)
  have hallv : all valid_wadrs
                 (map (fun (tmc : adrs * dgstblock * cntr) => tmc.`1)
...
(* ==========================================================================
   ...
   DISCHARGEABLE `A_wf_MA` ("member dfC0 never opened by the collection oracle")
   ...
   members <> dfC0) CAN satisfy.  The RHS S-TCR term is `S_TCR_C_Int_MA`, a genuine
...
(* ==========================================================================
   ...
   (`f = thfc(8*n)`).  Hence — with the FLAG fact `dfC0 <> 8*n` — the reduction NEVER
   opens the challenged member `dfC0`.  So `A_wf_MA` reduces to a condition on A's
   OWN direct `OC` queries (which, for the XMSS-MT hypertree leaf reduction, are
   pkco/trh/f — members 8*n*len / 8*n*2 / 8*n, all <> dfC0 by the FLAG facts).  This
