...
(* ==========================================================================
   ...
   ========================================================================== *)
(* ROUTE (D): ThC's INPUT is a NODE (dgstblock); its OUTPUT is the wide
   digest, assembled as the join of TWO members of the one collection.  The
   modelling fact therefore names both projections. *)
pred emb_thfc_ThC =
  forall (ps : pseed) (tw : adrs) (x : dgstblock * cntr),
    join_dgst (thfc (size (emb_in0 x)) ps (emb_tw tw) (emb_in0 x))
              (thfc (size (emb_in1 x)) ps (emb_tw tw) (emb_in1 x))
    = ThC ps tw x.`1 x.`2.
...
(* ROUTE (D): OC_emb answers ONE ThC query with TWO narrow queries, so the
   emulated FC transcript carries each tweak TWICE.  `dup2` names that, and
   because it commutes with `map` the invariant can be written
   `map emb_tw (dup2 Col.tws)` -- which lets `emb_disj_wgpidxs_lists` below
   apply VERBATIM at `adl' := dup2 Col.tws`.  Duplication is harmless to
   `disj_wgpidxs` anyway (it is `! has (mem ..) ..`, purely membership), but
   this way no downstream disjointness lemma needs restating at all. *)
op dup2 (l : adrs list) : adrs list = flatten (map (fun (x : adrs) => [x; x]) l).

lemma dup2_rcons (l : adrs list) (x : adrs) :
  dup2 (rcons l x) = rcons (rcons (dup2 l) x) x.
proof. by rewrite /dup2 map_rcons flatten_rcons -!cats1 -catA. qed.
...
(* ROUTE (D): generalised over the message type, so it serves BOTH the +C list
   (qC = wadrs * dgstblock) and the WOTS-TW list (qTW = wadrs * msgWOTS).  The
   proof only ever touches `.1`. *)
lemma all_valid_map_val ['a] (qs : (wadrs * 'a) list) :
  all valid_wadrs (map (fun (q : wadrs * 'a) => WAddress.val q.`1) qs).
proof.
...
(* ROUTE (D): this is the WOTS-TW side's query list, so its messages are WIDE
   digests -- `qTW`, not `qC`.  The lemma's content is the ADDRESS projection;
   only the element type had silently been sharing `qC`. *)
lemma oqs_adlO_eq (qs : qTW list) (ks : (pkWOTS * sigWOTS) list)
  ...
  = map (fun (q : qTW) => WAddress.val q.`1) qs.
proof.
...
module (R_bridge_WOTSTW (A : Adv_MnaCMA_WOTSC) : Adv_MEUFGCMA_WOTSTWESNPRF)
  ...
  module OC_emb : STCRC_WC.Col.Oracle_THFC = {
    ...
    (* ROUTE (D): emulate the ThC collection oracle with TWO narrow queries
       (the projections) joined outside the oracle.  Domain is ThC's -- a NODE
       and a counter; codomain is the WIDE digest. *)
    proc query(tw : adrs, x : dgstblock * cntr) : msgWOTS = {
      var y0, y1 : dgstblock;

      y0 <@ OC.query(emb_tw tw, emb_in0 x);  (* records `emb_tw tw` in OC *)
      y1 <@ OC.query(emb_tw tw, emb_in1 x);  (* ditto -- transcript grows by 2 *)
      tws <- rcons tws tw;

      return join_dgst y0 y1;
    }
  ...
  var qs : qTW list   (* R_multi_WOTSTW commits WIDE digests *)
...
proof.
  ...
  seq 6 6 : (={glob A, ps}
             ...
             /\ O_MEUFGCMA_WOTSTWESNPRF.ps{2} = ps{2}
             /\ FC.O_THFC_Default.tws{2}
                = map emb_tw (dup2 STCRC_WC.Col.O_THFC_Default.tws{1})
  ...
  + (* the emb per-query coupling: Col.query ~ OC_emb.query returns the SAME
    ...
    + proc; inline{2} FC.O_THFC_Default.query.
      ...
      move: harg => [-> ->] /=.
      by rewrite hpp (E FC.O_THFC_Default.pp{2} tw{2} x{2}) htws dup2_rcons !map_rcons.
  ...
  seq 4 3 : (={glob A, ps}
             ...
             /\ adlO{1} = map (fun (q : qTW) => WAddress.val q.`1) qs{1}
             /\ size ks{1} = size qs{1}
  ...
  + (* PROVEN 2026-07-08: committed-signing coupling.  Inline MM45's O.query
    ...
    while (={glob A, ps, k}
           ...
           /\ adlO{1} = map (fun (q : qTW) => WAddress.val q.`1) (take k{1} qs{1})
  ...
  + (* in range: couple the (identical) verify calls on equal arguments *)
    ...
    have hoq : map (fun (e : adrs * msgWOTS * pkWOTS * sigWOTS) => e.`1)
                   O_MEUFGCMA_WOTSTWESNPRF.qs{2}
             = map (fun (q : qTW) => WAddress.val q.`1) qs{1}.
    ...
    + rewrite hoq; move: pre => [#] *.
      have -> : FC.O_THFC_Default.tws{2} = map emb_tw (dup2 STCRC_WC.Col.O_THFC_Default.tws{1}) by smt().
