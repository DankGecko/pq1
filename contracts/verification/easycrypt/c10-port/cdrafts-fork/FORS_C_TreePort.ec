(* ==========================================================================
   ...
   per-hop extraction bounds) so the three terms have real meaning.
   ...
   SOUND DESIGN + faithful statements + honest partial proof.  TRH/TRCO are now
   closed; the OpenPRE obstruction remains explicit rather than hidden by a
   vacuous reduction.
   ...
   (F-EXTRACT-{OP,TRH,TRCO})  the three per-hop pRHL bounds (that each reduction's
       ...
       tree-hop re-derivations over the +C digest.  TRH and TRCO are now closed by
       concrete register-once reductions.  OP remains the sole labelled admit:
       unlike a TCR target, an OpenPRE target is sampled with a hidden preimage by
       the challenger, and the opaque deterministic `fkeygen` surface does not
       expose the leaf-key sampling that the original FORS_ES proof couples to it.
       The exact obstruction (including the actual-oracle index and opening-history
       gaps) is recorded above `extract_op`; it must not be replaced by a pointwise
       key-equality hypothesis, which would be false for real FORS.
...
(* ==========================================================================
   ...
   multi-week FORS_ES:2135-2815 +C-port compressed into flagged ops).  The TRH/TRCO
   extraction interfaces below are now grounded and their pRHLs are closed.  The OP
   enumeration/simulation interface is still incomplete: in particular, no current
   predicate connects `all_leaf_tweaks` plus the challenge images to `leaf_targets`,
   and `sim_sig_op` is not passed the preimages returned by `O.open`.

   NON-VACUITY ROLE.  TRH/TRCO now register honest targets unconditionally before
   the forger runs.  OP asks for `all_leaf_tweaks adz` once in `pick`; returning
   `[]` would force nrts=0 and an identically-zero RHS.  `enum_nondegen` rules out
   that one failure mode, but it does NOT establish the still-missing equality
   between the challenge-built O_OP_Default.ts and the pure `leaf_targets` list.
   ========================================================================== *)
...
(* ==========================================================================
   ...
   objects the F-EXTRACT bounds are meant to justify). *)
...
(* ==========================================================================
   ...
   then-open F-EXTRACT obligations: an individual statement was FALSE for some
   instantiation --
   ...
   (no single brk discharges all three vacuously), but each F-EXTRACT statement being
   ...
   simultaneously -- the op AND trco bounds are non-falsifiable AT ONCE (the
   ...
   -- that is the pigeonhole CONCLUSION and assuming it is the circularity trap.
   (Sanity: adding brk_structural alone does NOT let one `qed` an F-EXTRACT bound --
   the win-predicate game facts are simply absent from it.)
...
(* Static, non-circular, SATISFIABLE non-degeneracy of the enumeration ops.
   In particular, all_leaf_tweaks <> [] prevents the OpenPRE challenger from
   registering zero targets.  Witness realisation: the real FORS scheme has at
   least one leaf / k-1 trh nodes / one trco root per instance.  A hypothesis of
   the F-EXTRACT bounds (NOT an axiom); flagged for review. *)
pred enum_nondegen =
...
(* ==========================================================================
   ...
   trusted recompute/root_of walk), NONE asserting a preimage / a registered index:
     ...
     (LINJ) fold injectivity  -- croot_from_leaf is assumed injective in the
            leaf-image slot.
   AUDIT NOTE.  LC/LK are ordinary Merkle factorizations, but LINJ is stronger than
   Merkle structure: a path contains compression hashes, so fixed-width encoding alone
   does not prove the fold injective.  The original FORS_ES split instead defines the OP
   branch by equality of the forged and committed leaf images and sends a differing-image
   branch to TRH.  A faithful future refactor should use that classifier and remove LINJ.
   Until then this predicate is explicitly a flagged extra hypothesis, not something
   claimed to follow from the real Merkle construction.  It is grounded in
   recompute/root_of/f_lf, not extract_op/sim_sig_op, and is not an EasyCrypt `axiom`. *)
...
(* ==========================================================================
   op_registration  --  FLAGGED PURE leaf-target layout hypothesis.
   ==========================================================================
   THREE conjuncts, all about the HONEST leaf target list (grounded in the trusted
   ...
   These properties describe the intended real leaf layout and do not mention a win.
   IMPORTANT: unlike trh_registration in the closed TRH proof, this predicate is not
   yet connected by a pRHL invariant to the ACTUAL O_OP_Default.ts.  That missing
   challenge-list equality is R-INDEX above.  NOT an EasyCrypt `axiom`. *)
...
(* ==========================================================================
   ...
   PURE leaf-target set `leaf_targets ps ad pk` jointly satisfy four formulas with
   the shape of the game-independent SM_DT_OpenPRE conjuncts:
   ...
   This is deliberately only a PURE-list lemma.  It is not yet actual-oracle index
   fidelity: no premise equates `leaf_targets ps ad pk` to O_OP_Default.ts, whose
   images come from fresh `dop_in` samples.  The fifth conjunct `!opened` is also
   absent; moreover, `!covered` identifies SOME fresh forged leaf, while the current
   history-free `leaf_tw` need not select that leaf.  Both bridges belong in the
   missing pRHL, not in this lemma.  Every proved step here is grounded in
   op_preimage_core + op_registration + list-index facts and assumes no game win. *)
...
(* ==========================================================================
   sim_sig_op_fidelity  --  candidate simulator STRUCTURAL-fidelity hypothesis.
   ==========================================================================
   R_op is KEY-FREE (unlike the key-KNOWING R_trh/R_trco): its Osign cannot call
   `fsign sk` -- it has no sk -- so it opens the covered leaves through O.  The
   current procedure then DISCARDs every returned preimage and calls
   `sim_sig_op ps ad ys y`, whose type contains only public images.  A real FORS
   signature contains those opened secret-leaf values, so this interface is not yet
   a faithful implementation of the original reduction: the openings must be
   accumulated and passed to the simulator (or the simulator must call O.open).

   The predicate below states the desired byte-level equality only after the separate
   key-equality antecedent.  It is not itself a key-distribution coupling, and with the
   current independently sampled `ys` that antecedent may be unreachable, making the
   predicate vacuous.  It is retained to document the intended endpoint; it must not
   be counted as a discharged or independently load-bearing fidelity fact. *)
...
(* ==========================================================================
   ###  extract_op -- exact status of the remaining OpenPRE pRHL.
   ==========================================================================
   The pure mathematical witness is closed: op_preimage_core derives an f_lf
   preimage from the stated structural predicates, and op_extract_wins locates that
   pair in the PURE list `leaf_targets`.  Four independent bridges are nevertheless
   absent from the probability reduction:

   (R-KEY)  G_Tree gives A `(fkeygen ps ad).`1`, where fkeygen is an opaque
            deterministic op.  The OpenPRE side gives A `pk_of_leaves ps ad ys`,
            where ys is built from fresh hidden `dop_in` samples.  The A.forge call
            therefore requires
              (fkeygen ps ad).`1 = pk_of_leaves ps ad ys.
            The original FORS_ES proof establishes this by coupling EXPLICIT
            key-generation leaf samples to O.pick.  That sampling is sealed by the
            imported fkeygen op here.  A pointwise equality premise for arbitrary
            challenge ys would make the real Merkle public key independent of its
            leaves and is not a faithful structural hypothesis.

   (R-SIM)  O.sign calls O.open but discards every returned dgst.  sim_sig_op receives
            only the public image list, although a real FORS signature contains the
            opened secret leaf values.  The simulator must consume those openings;
            sim_sig_op_fidelity as currently typed cannot supply that data and its
            R-KEY antecedent can be unreachable.

   (R-INDEX) op_extract_wins indexes `leaf_targets ps ad pk`, but no premise relates
             that list (including order and images) to O_OP_Default.ts, which is
             actually built from `all_leaf_tweaks` and the sampled xs.  Thus the
             current pure lemma is not yet actual-oracle index fidelity.

   (R-OPEN)  `!covered` says that SOME forged leaf tuple is absent from signing
             history.  The current leaf_tw/extract_op is history-free, so it may
             select a different leaf that O.sign already opened.  A faithful port
             must track the opened global indices and select the first forged tuple
             absent from that history, as FORS_ES:2391-2411 does.

   Consequently the admit below is not an SMT residue and cannot be closed by the
   TRH/TRCO empty-pick + honest-sign transplant.  TCR permits O.query(tw,x) on a
   known first preimage; OpenPRE deliberately challenge-samples a HIDDEN preimage.
   Returning [] from R_op.pick would instead force nrts=0 and make the RHS
   identically zero.  A sound closure requires an upstream randomized/exposed leaf
   keygen interface plus the simulator, target-layout, and opening-history plumbing
   above.  No extract/win/collision-existence hypothesis is introduced here. *)
...
(* ==========================================================================
   The three per-hop EXTRACTION bounds (F-EXTRACT-OP / TRH / TRCO).  Each is the
   +C-variant re-derivation of one FORS_ES tree hop
   ...
   flip).  TRH/TRCO are proved below; OP remains the sole labelled admit and is
   not smuggled into the modules.  Premise `fverify_structural` is genuinely USED (the
...
proof.
  (* ========================================================================
     F-EXTRACT-OP (OPEN -- see the exact four-part obstruction above R_op).
     ========================================================================
     DISCHARGED as REAL pure lemmas (both empirically load-bearing -- dropping
     ...
     op_extract_wins with `unknown variable bop`):
       ...
       * PURE-LIST INDEX -- op_extract_wins: extract_op names the leaf target in
                           `leaf_targets` and proves range/budget/uniq/preimage there.

     NOT discharged: R-KEY, R-SIM, actual-oracle R-INDEX, and R-OPEN.  In
     particular, sim_sig_op_fidelity is conditional on R-KEY and is not an
     independent signing coupling. *)
  move=> struct nd bs los oreg simfid.
  admit. (* F-EXTRACT-OP: requires exposed randomized leaf keygen + faithful simulator,
            actual O.ts layout/index coupling, and history-selected !opened witness. *)
qed.
...
(* ==========================================================================
   ...
   partition (real, EXACT) + the three EXTRACT bounds.  TRH/TRCO are closed;
   the assembly is real, and its sole inherited admit is F-EXTRACT-OP above.
   ========================================================================== *)
...
(* (NV-2) STRUCTURAL non-degeneracy of the three RHS terms.  A never-querying
   ...
   still-open OP bound).  ANTI-VACUITY of the closed bound is additionally
   ...
   TRH (now CLOSED) -- R_trh registers `honest_nodes` ONCE before A runs.  Under
   trh_registration the list has distinct tweaks and the pure collision selector names
   an entry; the closed proof pins the actual O_TRH_Default.ts to that list.

   OP (still OPEN) -- R_op.pick returns `all_leaf_tweaks adz`, so enum_nondegen at
   least excludes the spurious nrts=0 construction.  This is not enough: no current
   invariant pins the challenge-built O_OP_Default.ts to `leaf_targets`, and no
   history-dependent selector proves the chosen entry unopened.  Thus no claim that
   the OP term is a completed non-vacuous reduction is made here. *)
