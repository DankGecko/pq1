...
(* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- *)
lemma grind_disj (ps : pseed) (ad : adrs) (m : msgWOTS) :
...
lemma query_eq_tw
  ...
  ~ R_int_WOTSTW(A, O_MEUFGCMA_WOTSTWESNPRF, FC.O_THFC_Default).O_wrap.query :
    ...
    /\ all (fun (t : adrs) => t \in FC.O_THFC_Default.tws{1} \/ exists b, t = emb_tw b)
           FC.O_THFC_Default.tws{2}
    /\ all (fun (q : adrs * msgWOTS * pkWOTS * (sigWOTS * cntr)) =>
...
rnd.
skip => &1 &2 [tws_R Hpre].
move: Hpre => [# htw0 hx0 hdf0 hy1 htwseq hd hwad0 hm0 hpseq had0eq hwadm hmm hinv1 hinv2 hinv3 hqs2 hwadsr hadeq hceq hps1eq had1eq htwsall hgateall].
