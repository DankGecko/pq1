...
**RESOLVED.** `honest_consistent : WellFormed sk → consistent sk` is proven kernel-clean in `SphincsCVerify/Verifier/HonestConsistent.lean` (closure `[propext, Classical.choice, Quot.sound]`, wired into the root, no sorry). All three increments done: (1) grind postconditions (`Spec/SignerPost.lean`), (2) reference signer completion (`Spec/Signer.lean` + `Spec/Treehash.lean`), (3) the `WellFormed`/`honest_consistent` assembly. `consistent`/`verify_signs` were NOT weakened; `honest_consistent` is the new top-level theorem supplying the honestly-keygen content `verify_signs` consumes.

**SCOPE / TRUST BASE (honest framing).** This is a *completeness* (usability) result — NOT in `theft_free`'s closure, so it is not a safety guarantee. It proves "spec-`Signer.sign` round-trips with spec-`verify`". Reading it as "firmware signatures are accepted on-chain" relies on two bridges: (a) spec-`verify` = the on-chain verifier — REAL (A3.1 `execC10Asm`=spec + the independent-KAT leg); (b) spec-`Signer.sign` = the firmware signer — **ASSUMED**. `Spec.Signer.sign` is hand-written, `noncomputable`, and was completed by reverse-engineering it to emit the verifier's own reconstruction shapes so the hypotheses discharge by construction; it is NOT cross-checked against the Rust/Python KAT oracle. So the proof cannot detect an unfaithful-but-self-consistent `sign`. **Follow-up (open):** anchor `Spec.Signer.sign` to an independent source (KAT oracle / `serialise`+deployed-verifier) to discharge bridge (b). **Full write-up of the two-specs picture + the anchoring options: [`two-specs-faithfulness.md`](two-specs-faithfulness.md).** The historical plan below is retained for reference.
