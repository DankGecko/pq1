...
| D5 | Rust→Lean (Charon/Aeneas) faithfulness | extracted specs | **PARTLY MITIGATED 2026-06-29; coverage gap sharpened 2026-07-02 (extracted-F3).** Two gates: (i) regen-diff (`make extract-*`, **11 targets** — sphincs-adrs/aa-userop/fors-index/wots-digits/txcore-rlp/fwmanifest-preimage/merkle-verify/wots-pkfromsig/tx-merkle/decode-item/hash-fns) catches un-re-extracted Rust changes; (ii) **differential** (`make verify-extract-differential`) EXECUTES the extracted Lean vs executed-Rust (`signed_preimage`, 6 vectors + negative control). Both tier C (corpus), not a ∀. **KNOWN UNGATED (extracted-F3):** the `SlotKdf` (invariant-#8 KDF byte-layout) and `PinState` (recovery-parser) ranks — both extracted from `domain/src/lib.rs` — have **NO regen-diff target**, so a Rust drift is not caught by (i). Confirmed 2026-07-02: the committed `SlotKdf/Funs.lean` IS stale vs the 2026-07-01 zeroize-audit edits — a fresh extraction adds a `zeroize(buf)` step + `let out ← … ; ok out` (functionally BENIGN: zeroize of a local does not change `out`, so `slot_entropy_hashes_canonical_preimage` still describes reality). Re-committing needs a re-prove of the rank against the new body (a `step`-through-zeroize adjustment) on the v4.30 extracted toolchain — a **tracked follow-up** (`docs/work-todo.md` §35). A byte-exact regen target could not be reconstructed 2026-07-02 (the recorded flags reproduce the logic but the aeneas import-namespace output differs — `Slotkdf.*` vs the committed `Sk.*` — so a fabricated target would spuriously always-DRIFT; recovering the exact original invocation is part of the follow-up). Charon still has no foundational soundness proof (cited-TCB). | roadmap T1.3, `ExtractDiffCheck.lean`, work-todo §35 |
