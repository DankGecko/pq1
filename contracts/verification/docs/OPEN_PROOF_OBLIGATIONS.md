...
* `Wallet/Invariants.lean::create2_address_chain_independent` (I-7) — pins the
  ...
  **UPDATE 2026-07-17 — `ich1 = ich2` TCB decomposed.**
  ...
  separate post-deploy write) + the homogeneous `impl1 = impl2` premise.
  `initCodeHash_eq_of_impl_eq` is `subst; rfl` (= `congrArg`): it witnesses the
  `def`'s SHAPE (no chainId argument), NOT a byte-level proof that the real
  deployed initCode is chain-free — that "no chainId in the real initCode" fact
  stays cited-TCB, source-verified against `PQSmartWalletFactory.sol:93` (no
  Solidity→Lean initcode-byte extraction was done). What improves is the TCB
  SHAPE, not the hypothesis count: the cited-TCB surface becomes TWO HOMOGENEOUS
  deterministic-deployment address identities
