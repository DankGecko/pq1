...
| # | ID | Class | Sev | Target | One-line |
|---|----|-------|-----|--------|----------|
| 3b-1 | msend-op0-delegatecall-refusal-outside-kani-surface | G3 | low | multisend.rs harnesses vs eip712/safe/multi_send.rs:126 | **The core multiSend safety property — per-record `op==0` (never blind-sign a nested DELEGATECALL) — has ZERO Kani coverage; the refusal lives only in the un-extracted, un-Kani'd `summarize()`** |
...
- **Fix.** Extract the `op==0` gate into `pqsigner-tx` + a Kani harness (`walk with any op!=0 ⟹ verdict rejects`), or add an explicit scope note to the module header that per-record DELEGATECALL-refusal is out of Kani reach (covered by `summarize` + the on-chain revert). Filed in work-todo §35 (batch item, needs a real `cargo kani` run).

### 3b-2 … 3b-5 (recorded, filed)

- **3b-2 (V5, low)** — grep-verified two call sites (safe_display.rs:424 renderer + :873 gate) and the verdict-before-gate ordering; the disclosure comment's premises are false though the payload-length-derived overflow-freedom conclusion still holds. Cheap comment correction (filed).
