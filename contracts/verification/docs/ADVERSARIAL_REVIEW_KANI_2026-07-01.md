...
- **Fix.** Extract the `op==0` gate into `pqsigner-tx` + a Kani harness (`walk with any op!=0 ⟹ verdict rejects`), or add an explicit scope note to the module header that per-record DELEGATECALL-refusal is out of Kani reach (covered by `summarize` + the on-chain revert). **Status 2026-07-01:** the module-header scope note is added (multisend.rs:22-37, stopgap); the real close — a Kani harness — is the work-todo §35 (k) batch item, **in progress this session**.
