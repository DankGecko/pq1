...
| I-3 No reset (structural) | `no_reset_path` + Storage no-decrease lemmas | `Wallet/Invariants.lean` | ✅ Closed (per-mutator; now incl. cross-counter preservation; covers the 5 counter mutators bumpBootstrap/bumpSlot/setOffchain/addOwner/removeOwner. `tryInitialize` — a 6th counter mutator that zeroes the counters at construction — is excluded by the one-shot `nextOwnerIndex == 0` init guard, NOT by `no_reset_path`. `no_reset_path` is a per-method monotonicity statement, not an assembled reachable-state theorem.) |
| I-4 Bootstrap unremovable | `cannot_remove_bootstrap` / `bootstrap_unremovable` | `Wallet/Invariants.lean` / `Wallet/MultiOwnable.lean` | ✅ Closed (per-mutator + one-shot init guard; the owner-set-never-empty guarantee is per-mutator preservation of `ownerAtIndex 0` plus `owner_set_nonempty_after_init` on the `initialised` state, NOT an assembled reachable-state theorem) |
| I-5 Combined cap (per-method + inductive) | `combinedCap_preserved_by_*`, `combinedCap_inductive` | `Wallet/Invariants.lean` | ✅ Closed |
| I-6 EIP-1271 forbids bootstrap | `eip1271_forbids_bootstrap` + `test/halmos/HalmosIsValidSignature.t.sol` | `Wallet/Invariants.lean` (model in `Wallet/IsValidSignature.lean`) | ✅ Model-proven AND now bytecode-discharged (2026-06-29): `HalmosIsValidSignature` (3 rules, BOTH profiles) covers the net-new EIP-1271 surface the A3.2 harnesses skip. `check_isValidSignature_forbids_bootstrap` is a clean bytecode **∀** over `hash` + symbolic verifier (ownerIndex 0 rejected before the `ownerAtIndex` read + keccak nesting); `_nonbypass_slot1` (magic ⇒ verifier-true) + `_no_counter_bump` (view-only) over the **installed** slot 1. Disclosed ceiling (= A3.2): the non-bypass/view-only rules use the installed index as a concrete rep — a symbolic unset index `NotConcreteError`s on the `ownerAtIndex` dynamic getter, so those two are per-property B, not symbolic-∀ over all indices. No `solidityEip1271_compiles_correctly` bridge axiom; the Halmos session stands alone (as the per-property `HalmosValidateUserOp` rules do for A3.2). keccak (`replaySafeHash`) uninterpreted. |
| I-7 CREATE2 chain-independent | `create2_address_chain_independent`, `create2_salt_definition` | `Wallet/Invariants.lean` | ✅ Salt-preimage chain-freeness is kernel-proven (`create2_salt_definition`, `create2_address_chain_independent` — both reduce the salt to `sha256(masterPkSeed‖masterPkRoot)`, no chain id). Address-level chain-independence is CONDITIONAL: the distinct `create2Address_chain_independent` (camelCase — NOT the snake_case salt-only theorem above) requires the EVM-TCB facts `d1=d2` (singleton-deployer factory deployed to one address per chain) and `ich1=ich2` (chain-free `keccak256(initCode)`); it is not an unconditional kernel theorem. |
