...
| I-6 EIP-1271 forbids bootstrap | `eip1271_forbids_bootstrap` | `Wallet/Invariants.lean` (model in `Wallet/IsValidSignature.lean`) | ✅ Model-proven, but NOT bytecode-bridged: unlike the sibling deployed surfaces — validateUserOp (`theft_free_bytecode`) and factory (`factory_squat_defence_bytecode`), each also covered by a Halmos/Kontrol session — the EIP-1271 path has no `*_bytecode` corollary and no symbolic Halmos/Kontrol harness. The kernel proof binds only the Lean model. |
