...
| 4 | Firmware Kani bounded proofs | 161 active `#[kani::proof]` harnesses in 26 files, pinned by exact file/function identity | Nightly/default tiers plus a curated mutation manifest | 153 harnesses in 20 files reside in mutation-enrolled files; eight harnesses in six files do not. Default/nightly runs 38/41 mutation groups; three are full-only. The three ERC-7730 Phase-D parameter harnesses were individually verified with Kani 0.67.0; no fresh full Kani campaign was run. Bounded harnesses are not architecture-wide proofs. |
