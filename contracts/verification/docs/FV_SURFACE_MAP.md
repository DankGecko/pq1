...
**Snapshot: 2026-07-23** (reconciled after the 2026-07-19 deep review; previously 2026-07-15). This map owns the current inventory of proof and
...
| 4 | Firmware Kani bounded proofs | 173 active `#[kani::proof]` harnesses in 27 files (census-locked by `scripts/kani_census.py` + `kani_census.lock.json`) | Nightly/default tiers plus a curated mutation manifest | 162 harnesses in 21 files reside in mutation-enrolled files; 11 harnesses in six files do not. Default/nightly runs 40/43 mutation groups; three are full-only. NOTE (2026-07-23): GitHub CI evidence remains suspended by the billing outage, so this row is local-run evidence. The three ERC-7730 Phase-D parameter harnesses were individually verified with Kani 0.67.0; the current full Kani campaign is reported separately by the closure evidence. Bounded harnesses are not architecture-wide proofs. |
