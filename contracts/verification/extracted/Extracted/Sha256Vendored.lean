/- §33 — version bridge, SHA-256 leg: the FIPS 180-4 reference
   implementation from SphincsCVerify, VENDORED.
   ...
   (Aeneas's toolchain). The two can't share a lake build, so — exactly
   as `SpecVendored.lean` does for ByteVec/Adrs — this file VENDORS
   `lean/SphincsCVerify/Spec/Sha256Impl.lean` verbatim (a faithful
   transcription, same names / structure / section comments), so the
   computable `sha256_pure` adapter in `Sha256Pure.lean` runs the
   actual on-chain spec's SHA-256 rather than a local restatement.

   Fidelity of the vendored copy to the source is CI-guarded:
   `make verify-spec-vendored-fidelity` diffs the definitions below
   against
   `contracts/verification/lean/SphincsCVerify/Spec/Sha256Impl.lean`.

   Like the source, this file is mathlib-free (core `BitVec` only; no
   imports beyond the prelude). The original header comment follows. -/
